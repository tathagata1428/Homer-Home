...
function historyIsPrefix(prefix, whole) {
  const left = asArray(prefix);
  const right = asArray(whole);
...
async function primeSharedStateFromIncomingTurn(redisFetch, keys, mode, messages, options = {}) {
  ...
  const [previousHistory, profile, memories, files, fileLibrary, customFiles, journal, storedSyncMeta] = await Promise.all([
  ...
  const nextFiles = (historyChanged || profileChanged || memoriesChanged)
    ? buildContextFiles({
        profile: nextProfile,
  ...
  const syncMeta = computeJoeySyncMeta({
    ...
    lastDeviceId: deviceId || (storedSyncMeta && storedSyncMeta.lastDeviceId) || '',
    lastDeviceLabel: deviceLabel || (storedSyncMeta && storedSyncMeta.lastDeviceLabel) || '',
    lastDeviceSyncedAt: updatedAt
  });

  const journalEntries = [];
  if (historyChanged) {
    const prior = asArray(previousHistory);
    if (historyIsPrefix(prior, mergedHistory)) {
      mergedHistory.slice(prior.length).forEach((item) => {
    ...
    } else if (!mergedHistory.length && clearedAt > 0) {
      ...
      journalEntries.push(buildJournalEntry('history-rewrite', {
        text: JSON.stringify(mergedHistory.slice(-8)),
        source: 'chat-turn'
      }));
    }
  }
  if (memoriesChanged) {
    nextMemories.slice(Math.max(0, nextMemories.length - asArray(deterministic.memories).length)).forEach((item) => {
      journalEntries.push(buildJournalEntry('memory-learned', {
        text: item.text,
        category: item.category,
        source: item.source || 'deterministic-learn',
        ts: item.ts
      }));
    });
  }
  ...
  if (historyChanged || profileChanged || memoriesChanged) writes.push(saveRedisJson(redisFetch, keys.FILES_KEY, nextFiles));
  if (journalEntries.length) {
    writes.push(saveRedisJson(redisFetch, keys.JOURNAL_KEY, asArray(journal).concat(journalEntries).slice(-MAX_JOURNAL)));
  }
...
export async function handleJoeyGatewayRequest(req, res, options) {
  ...
  const { MEMORY_KEY, PROFILE_KEY, HISTORY_KEY, FILES_KEY } = keys;
  ...
  const filesBundle = primedState.files && typeof primedState.files === 'object' ? { ...primedState.files } : {};
