...
export async function handleJoeyGatewayRequest(req, res, options) {
  ...
  const { messages, passphrase, systemPromptOverride, savedQuotesContent, chatClearedAt, deviceId, deviceLabel, providerHint } = req.body || {};
  ...
  const redisFallbackFetch = createRedisFetch();
  ...
  try {
    ...
    if (!isValid && passphrase) {
      if (supabaseAdminEnabled) {
        ...
        if (!isValid) {
          isValid = await verifyJoeyPassphrase(passphrase, redisFallbackFetch);
        }
        if (isValid && ownerId) {
          redisFetch = createSupabaseRedisFetch(createAdminClient(), ownerId);
        }
      } else {
        if (!redisUrl || !redisToken || !redisFallbackFetch) {
          return res.status(500).json({ error: 'Redis not configured' });
        }
        isValid = await verifyJoeyPassphrase(passphrase, redisFallbackFetch);
        if (isValid) redisFetch = redisFallbackFetch;
      }
    }
    ...
    const provider = typeof config.getProviderConfig === 'function'
      ? config.getProviderConfig({ mode, env: process.env, providerHint })
      : {};
    ...
    for (const modelName of modelsToTry) {
      ...
      if (upstream) {
        ...
        lastErrorStatus = 502;
        lastErrorBody = 'Gateway unreachable';
      }
      upstream = null;
      const retryable = lastErrorStatus === 429 || lastErrorStatus === 404 || /rate_limit|tokens|rate limit|not found/i.test(lastErrorBody);
      if (!retryable || modelName === modelsToTry[modelsToTry.length - 1]) break;
    }

    // If primary gateway failed entirely, try fallback gateway config (e.g. kilo.ai when local tunnel is down)
    if (!upstream && typeof config.getFallbackGatewayConfig === 'function') {
      const fbCfg = config.getFallbackGatewayConfig({ mode, env: process.env });
      if (fbCfg && fbCfg.gatewayUrl && fbCfg.primaryModel) {
        const fbHeaders = { 'Content-Type': 'application/json' };
        if (fbCfg.gatewayToken) fbHeaders.Authorization = 'Bearer ' + fbCfg.gatewayToken;
        upstream = await fetch(buildChatUrl(fbCfg.gatewayUrl), {
          method: 'POST',
          headers: fbHeaders,
          body: JSON.stringify({ model: fbCfg.primaryModel, messages: finalMessages, stream: true, max_tokens: 1800 })
        }).catch(() => null);
        if (upstream && upstream.ok) {
          activeModel = fbCfg.primaryModel;
        } else {
          if (upstream) {
            lastErrorStatus = upstream.status;
            lastErrorBody = await upstream.text().catch(() => upstream.statusText);
