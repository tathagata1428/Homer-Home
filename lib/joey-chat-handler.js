...
function buildMemoriesText(memories) {
  ...
  memories.forEach((memory) => {
    const category = (memory && memory.category) || 'general';
    if (!groups[category]) groups[category] = [];
    groups[category].push(memory.text);
  });

  const lines = [];
  Object.entries(groups).forEach(([category, texts]) => {
...
export async function handleJoeyGatewayRequest(req, res, options) {
  ...
  const { messages, passphrase, systemPromptOverride } = req.body || {};
  ...
  const [memories, profile, historyMessages, filesBundle] = await Promise.all([
  ...
  const backfillMessages = getBackfillMessages(historyMessages, messages, backfillLimit);
