...
const MAX_UPLOAD_DOC_FILES = 12;
const MAX_UPLOAD_LIBRARY_ITEMS = 24;
const MAX_UPLOAD_TEXT_CHARS = 12000;
const MAX_UPLOAD_EXCERPT_CHARS = 280;
...
function normalizeFileRecord(file) {
  ...
  const rawExtractedText = String(file && (file.extractedText || file.content || '') || '').trim();
  const extractedText = rawExtractedText.slice(0, MAX_UPLOAD_TEXT_CHARS);
  ...
  return {
    ...
    excerpt: text(file && file.excerpt) || extractedText.slice(0, MAX_UPLOAD_EXCERPT_CHARS),
    extractedText,
    textLength: Number(file && file.textLength) || rawExtractedText.length || extractedText.length || 0,
    docKey
  };
}

function buildFilesIndexMd(fileLibrary, customFiles, generatedAt) {
  const files = Array.isArray(fileLibrary) ? fileLibrary.map(normalizeFileRecord).sort((a, b) => compareTs(a.uploadedAt, b.uploadedAt)) : [];
  const custom = (customFiles && typeof customFiles === 'object' && !Array.isArray(customFiles)) ? customFiles : {};
  const customNames = Object.keys(custom)
    .map((name) => text(name))
    .filter((name) => name && !SYSTEM_FILE_NAMES.has(name) && !/^Uploads\//i.test(name) && !/^Preserved\//i.test(name))
...
export function buildUploadDocFile(file, generatedAt) {
  const normalized = normalizeFileRecord(file);
  const lines = fileHeader(normalized.docKey, generatedAt, ['Remembered uploaded file for agent reuse']);
  lines.push(mdSection('Metadata', bulletLines([
    'Name: ' + normalized.name,
    normalized.mimeType ? 'Type: ' + normalized.mimeType : '',
    normalized.size ? 'Size: ' + formatBytes(normalized.size) : '',
    normalized.uploadedAt ? 'Uploaded: ' + normalized.uploadedAt : '',
    normalized.processedAt ? 'Processed: ' + normalized.processedAt : '',
    normalized.sha256 ? 'SHA256: ' + normalized.sha256 : '',
    normalized.driveUrl ? 'Drive URL: ' + normalized.driveUrl : '',
    normalized.downloadUrl ? 'Download URL: ' + normalized.downloadUrl : '',
    normalized.textLength ? 'Text Length: ' + normalized.textLength + ' chars' : '',
    normalized.notes ? 'Notes: ' + normalized.notes : ''
  ])));
  lines.push(mdSection('Excerpt', normalized.excerpt ? [normalized.excerpt] : []));
  lines.push(mdSection('Extracted Text', normalized.extractedText ? [normalized.extractedText] : ['_No extracted text stored._']));
  return {
    name: normalized.docKey,
    content: lines.filter(Boolean).join('\n').replace(/\n{3,}/g, '\n\n').trim() + '\n'
  };
}

export function compactFileLibraryEntries(fileLibrary) {
  const files = Array.isArray(fileLibrary) ? fileLibrary.map((file) => {
    const normalized = normalizeFileRecord(file);
    ...
    next.mimeType = normalized.mimeType;
    next.size = normalized.size;
    next.sha256 = normalized.sha256;
    next.uploadedAt = normalized.uploadedAt;
    next.processedAt = normalized.processedAt;
    next.driveUrl = normalized.driveUrl;
    next.downloadUrl = normalized.downloadUrl;
    next.driveFileId = normalized.driveFileId;
    next.notes = normalized.notes;
    next.excerpt = normalized.excerpt;
    next.textLength = normalized.textLength;
    return next;
  }).sort((a, b) => compareTs(a.uploadedAt, b.uploadedAt)) : [];
  return files.slice(-MAX_UPLOAD_LIBRARY_ITEMS);
}

function buildUploadDocs(fileLibrary, existingFiles, generatedAt) {
  const files = Array.isArray(fileLibrary) ? fileLibrary.map(normalizeFileRecord).sort((a, b) => compareTs(a.uploadedAt, b.uploadedAt)) : [];
  const currentFiles = (existingFiles && typeof existingFiles === 'object' && !Array.isArray(existingFiles)) ? existingFiles : {};
  const out = {};
  files.slice(-MAX_UPLOAD_DOC_FILES).forEach((file) => {
    if (!file.extractedText && typeof currentFiles[file.docKey] === 'string' && currentFiles[file.docKey].trim()) {
      out[file.docKey] = currentFiles[file.docKey];
      return;
    }
    const uploadDoc = buildUploadDocFile(file, generatedAt);
    out[uploadDoc.name] = uploadDoc.content;
  });
  return out;
}

export function mergeDerivedFileContext(existingFiles, fileLibrary, customFiles, generatedAt) {
  const files = { ...((existingFiles && typeof existingFiles === 'object' && !Array.isArray(existingFiles)) ? existingFiles : {}) };
  const timestamp = text(generatedAt) || new Date().toISOString();
  const uploads = buildUploadDocs(fileLibrary, files, timestamp);
  const custom = (customFiles && typeof customFiles === 'object' && !Array.isArray(customFiles)) ? customFiles : {};

  Object.keys(files).forEach((name) => {
    if (/^Uploads\//i.test(name)) delete files[name];
    if (/^Preserved\//i.test(name)) delete files[name];
  });
  files['FilesIndex.md'] = buildFilesIndexMd(fileLibrary, custom, timestamp);
  Object.entries(uploads).forEach(([name, content]) => {
    files[name] = content;
  });
  Object.entries(custom).forEach(([name, content]) => {
    const safeName = String(name || '').trim();
    if (!safeName || SYSTEM_FILE_NAMES.has(safeName) || /^Uploads\//i.test(safeName) || /^Preserved\//i.test(safeName)) return;
    if (typeof content !== 'string' || !content.trim()) return;
    files[safeName] = content.trim();
  });
  return files;
}

function sanitizeCustomFileEntries(customFiles) {
  const out = {};
  const source = (customFiles && typeof customFiles === 'object' && !Array.isArray(customFiles)) ? customFiles : {};
...
function extractManualAppendix(content) {
  const value = String(content || '');
  if (!value.trim()) return '';
  const match = value.match(/(?:^|\n)(##\s+(?:Manual Additions|Manual Notes|User Notes|Appended Notes|Preserved Notes)\b[\s\S]*)$/i);
  return match ? match[1].trim() : '';
}

function appendManualAppendix(generated, appendix) {
  const base = String(generated || '').trim();
  const notes = String(appendix || '').trim();
  if (!notes) return base ? (base + '\n') : '';
  if (base.includes(notes)) return base ? (base + '\n') : '';
  return (base ? (base + '\n\n') : '') + notes + '\n';
}

export function preserveGeneratedContextFiles(existingFiles, generatedFiles, customFiles, generatedAt) {
  const previous = (existingFiles && typeof existingFiles === 'object' && !Array.isArray(existingFiles)) ? existingFiles : {};
  const nextFiles = { ...((generatedFiles && typeof generatedFiles === 'object' && !Array.isArray(generatedFiles)) ? generatedFiles : {}) };
  const nextCustomFiles = sanitizeCustomFileEntries(customFiles);

  Object.keys(nextFiles).forEach((name) => {
    if (!SYSTEM_FILE_NAMES.has(name)) return;
    const previousContent = String(previous[name] || '').trim();
    const generatedContent = String(nextFiles[name] || '').trim();
    if (!previousContent || !generatedContent || previousContent === generatedContent) return;
    const appendix = extractManualAppendix(previousContent);
    if (appendix) {
      nextFiles[name] = appendManualAppendix(generatedContent, appendix);
    }
  });

  Object.entries(nextCustomFiles).forEach(([name, content]) => {
...
function buildAgentContextMd(profile, memories, history, tasks, fileLibrary, generatedAt, scope) {
  ...
  const identityAnchors = [
    text(profile.name) ? 'Name: ' + text(profile.name) : '',
    text(profile.nickname) ? 'Preferred name: ' + text(profile.nickname) : '',
    text(profile.profession) ? 'Role: ' + text(profile.profession) : '',
    text(profile.location) ? 'Location: ' + text(profile.location) : '',
    text(profile.timezone) ? 'Timezone: ' + text(profile.timezone) : '',
    text(profile.communication_style) ? 'Communication style: ' + text(profile.communication_style) : '',
    text(profile.current_mood) ? 'Current mood: ' + text(profile.current_mood) : '',
    ...list(profile.interests).slice(0, 6).map((item) => 'Interest: ' + item)
  ].filter(Boolean);
  ...
  const lines = fileHeader(
    ...
    workScope
      ...
      : ['Compact operational context for Joey and Joey Nemo', 'Read this first before deeper PARA files', 'Bias toward making the assistant feel specific to this user']
  );
  lines.push(mdSection('Identity Anchors', bulletLines(identityAnchors)));
...
export function buildContextFiles(data) {
  ...
  const existingFiles = data && data.existingFiles && typeof data.existingFiles === 'object' && !Array.isArray(data.existingFiles) ? data.existingFiles : {};
  const scope = text(data && data.scope).toLowerCase() === 'work' ? 'work' : 'personal';
  const customFileEntries = sanitizeCustomFileEntries(data && data.customFiles);
  const generatedAt = text(data && data.generatedAt) || new Date().toISOString();
  ...
  return {
    ...
    'FilesIndex.md': buildFilesIndexMd(fileLibrary, data && data.customFiles, generatedAt),
    'User.md': userMd,
    'Memory.md': memoryMd,
    'Tasks.md': formatTasks(tasks, generatedAt),
    'HistorySummary.md': historySummaryMd,
    ...buildUploadDocs(fileLibrary, existingFiles, generatedAt),
    ...customFileEntries
  };
}
