...
export function mergeDerivedFileContext(existingFiles, fileLibrary, customFiles, generatedAt) {
  ...
  files['FilesIndex.md'] = buildFilesIndexMd(fileLibrary, timestamp);
...
export function preserveGeneratedContextFiles(existingFiles, generatedFiles, customFiles, generatedAt) {
  ...
  const stamp = text(generatedAt) || new Date().toISOString();
  const archiveStamp = stamp.replace(/[^0-9TZ]/g, '').replace(/[:.-]/g, '');
  ...
  Object.keys(nextFiles).forEach((name) => {
    ...
    const archiveName = 'Preserved/' + archiveStamp + '--' + safeSlug(name, 'context') + '.md';
    if (!nextCustomFiles[archiveName]) nextCustomFiles[archiveName] = previousContent;
  });
