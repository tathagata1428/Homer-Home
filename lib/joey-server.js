...
export async function fetchWithRedirects(startUrl, options) {
  ...
  for (let index = 0; index < maxRedirects; index += 1) {
    ...
    // Use Promise.race for body read — AbortController may not abort body reads
    // in all CF Worker environments, so we race against an explicit timeout promise.
    {
      const elapsed2 = Date.now() - startedAt;
      const bodyRemainingMs = timeoutMs ? Math.max(1, timeoutMs - elapsed2) : 0;
      let bodyTimeoutId = null;
      const bodyTextPromise = response.text();
      const bodyTimeoutPromise = bodyRemainingMs > 0
        ? new Promise((_, reject) => { bodyTimeoutId = setTimeout(() => reject(new Error('Body read timed out')), bodyRemainingMs); })
        : null;
      try {
        responseText = bodyTimeoutPromise
          ? await Promise.race([bodyTextPromise, bodyTimeoutPromise])
          : await bodyTextPromise;
      } catch (bodyError) {
        if (bodyTimeoutId) clearTimeout(bodyTimeoutId);
        if (timeoutId) clearTimeout(timeoutId);
        if (bodyError && /timed out/i.test(String(bodyError.message || ''))) {
          return { ok: false, status: 504, error: 'Request timed out (body)', redirectChain, text: '' };
        }
        throw bodyError;
      }
      if (bodyTimeoutId) clearTimeout(bodyTimeoutId);
