...
import { buildUploadDocFile, compactFileLibraryEntries, mergeDerivedFileContext } from './context-files.js';
...
export default async function handleGdriveFile(req, res) {
  ...
  try {
    ...
    const rawExtractedText = String(extractedText || '').trim();
    const normalizedText = rawExtractedText.slice(0, MAX_UPLOAD_TEXT_CHARS);
    ...
    const record = {
      ...
      textLength: rawExtractedText.length || normalizedText.length || 0
    };
    ...
    const compactLibrary = compactFileLibraryEntries(library);
    ...
    const nextFiles = mergeDerivedFileContext(currentFiles, compactLibrary, customFiles, uploadedAt);
    if (normalizedText) {
      const uploadDoc = buildUploadDocFile({ ...record, extractedText: normalizedText }, uploadedAt);
      nextFiles[uploadDoc.name] = uploadDoc.content;
    }
    ...
    await Promise.all([
      saveRedisJson(redisFetch, FILE_LIBRARY_KEY, compactLibrary),
