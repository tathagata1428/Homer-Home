...
export async function runLearnStep({ mode, messages, redisFetch, keys }) {
  ...
  const gatewayUrl = String(
    isWork
      ...
      : (process.env.OC_PERSONAL_GATEWAY_URL || process.env.OC_GATEWAY_URL || process.env.NEMOCLAW_GATEWAY_URL || 'http://localhost:11434')
  ).trim().replace(/\/+$/, '');
  const gatewayToken = String(
    isWork
      ? (process.env.OC_WORK_GATEWAY_TOKEN || process.env.OC_GATEWAY_TOKEN || '')
      : (process.env.OC_PERSONAL_GATEWAY_TOKEN || process.env.OC_GATEWAY_TOKEN || process.env.NEMOCLAW_GATEWAY_TOKEN || '')
  ).trim();
  const model = String(
    isWork
