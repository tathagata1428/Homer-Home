...
export async function runLearnStep({ mode, messages, redisFetch, keys }) {
  ...
  const gatewayUrl = String(
    process.env.OC_PERSONAL_GATEWAY_URL || process.env.OC_GATEWAY_URL || 'http://localhost:11434'
  ).trim().replace(/\/+$/, '');
  const gatewayToken = String(
    process.env.OC_PERSONAL_GATEWAY_TOKEN || process.env.OC_GATEWAY_TOKEN || ''
  ).trim();
  const model = String(process.env.OC_MODEL || 'kimi-k2.6:cloud').trim().replace(/^kimi-k2\.5(:cloud)?$/i, 'kimi-k2.6:cloud');
...
Return ONLY this JSON:
{
  ...
  if (canUseModel) {
    ...
    const learnTimeout = setTimeout(() => learnAbort.abort(), 12000);
    try {
