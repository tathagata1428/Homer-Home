...
export function buildJoeySyncDrift(redisMeta, driveMeta) {
  ...
  const buckets = ['profile', 'memories', 'history', 'files', 'fileLibrary', 'customFiles', 'journal', 'bundle'];
  const drift = {};
  let mismatchCount = 0;
  ...
  buckets.forEach((key) => {
    ...
    if (available && !match) mismatchCount += 1;
    drift[key] = {
      available,
      match,
      redisHash,
  ...
  return {
    inSync: mismatchCount === 0 && buckets.some((key) => drift[key].available),
    mismatchCount,
    buckets: drift
  };
}
