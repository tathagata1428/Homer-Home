...
export function getJoeyContextKeys(mode) {
  ...
  return {
    ...
    SYNC_META_KEY: prefix + ':sync_meta'
  };
}

export function getJoeyContextEnv(mode, env) {
  const vars = env && typeof env === 'object' ? env : process.env;
  // Also try trimmed key names in case of CF dashboard trailing-space issues
  const get = (k) => {
    if (vars[k] != null) return String(vars[k]).trim();
    const found = Object.entries(vars).find(([ek]) => ek.trim() === k);
    return found ? String(found[1]).trim() : '';
  };
  if (normalizeJoeyMode(mode) === 'work') {
    return get('JOEY_WORK_CONTEXT') || get('JOEY_CONTEXT');
  }
  return get('JOEY_PERSONAL_CONTEXT') || get('JOEY_CONTEXT');
}
