/* ====================================================================
 * Homer Backup Control Panel  v20260604a
 *
...
(function () {
  ...
  /* ── Vault IDB reader (homer-vault-idb) ────────────────────────── */
  ...
  /* ── Car IDB reader (homer-car-db — includes file attachment blobs) */
  function collectCarIdb() {
    return new Promise(function (resolve) {
      try {
        var req = indexedDB.open('homer-car-db', 1);
        req.onupgradeneeded = function (e) { e.target.result.createObjectStore('car'); };
        req.onsuccess = function (e) {
          try {
            var getReq = e.target.result.transaction('car', 'readonly').objectStore('car').get('main');
            getReq.onsuccess = function () { resolve(getReq.result !== undefined ? getReq.result : null); };
  ...
  function runFullBackup(onStatus) {
    ...
    return Promise.all([collectIdb(), collectCarIdb()]).then(function (results) {
      var idbData = results[0];
      var carIdb  = results[1];
      var snap = Object.assign({}, collectLS(), idbData);
      if (carIdb !== null) snap['homer-car-data'] = carIdb; // full car data incl. file blobs
  ...
  function renderPanel() {
    ...
    if (stats) {
      ...
      if (c.fieldState)               chips.push(c.fieldState + ' synced fields (car, expenses, notes, kanban, life goals, habits, secrets\u2026)');
      if (c.habits)                   chips.push(c.habits + ' habits');
      if (c.habitCompletions)         chips.push(c.habitCompletions + ' habit completions');
      if (c.tasks)                    chips.push(c.tasks + ' tasks');
      if (c.focusSessions)            chips.push(c.focusSessions + ' focus sessions');
      if (stats.hasCarIdb)            chips.push('car IDB \u2714 (file attachments)');
      if (chips.length) {
    ...
    panelEl.innerHTML =
      ...
      + '<p style="margin:0;font-size:.74rem;color:#64748b;">localStorage &bull; vault IDB &bull; car IDB (file attachments) &bull; habits, expenses, notes, car tracker, kanban, life goals, journal, secret notes, inbox &bull; Supabase field state &bull; focus sessions &bull; Joey AI memories &amp; history</p></div>'
