...
(function () {
  ...
  function initSupabaseDataSync(){
    ...
    function mergeJournal(local,remote){
      var byId={};
      (Array.isArray(remote)?remote:[]).forEach(function(e){if(e&&e.id!=null)byId[String(e.id)]=Object.assign({},e);});
    ...
    function mergeKanbanBlob(local,remote){
      function mergeArr(loc,rem){
        ...
        if(loc&&loc.length>0){
          ...
          (rem||[]).forEach(function(x){byId[x.id]=x;});
        }
        return Object.keys(byId).map(function(k){return byId[k];});
      }
    ...
    function mergeLifeGoalsArr(local,remote){
      ...
      if(local&&local.length>0){
        ...
        // Local is empty: accept everything from remote (fresh install / first sync)
        (remote||[]).forEach(function(g){byId[g.id]=g;});
      }
      return Object.keys(byId).map(function(k){return byId[k];});
    }
