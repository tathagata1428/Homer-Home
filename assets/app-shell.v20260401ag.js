...
(function(){
  ...
  function ensureVaultServerMirror(){
    var syncPass = getActiveSyncPass();
    var localSaveTs = parseInt(localStorage.getItem(BACKUP_LOCAL_SAVE_TS_KEY) || '0', 10) || 0;
  ...
  window._homerBackupEverythingToDb = function(){
    var p = getActiveSyncPass();
    if(!p) return Promise.reject(new Error('Shared DB sync is available only for Bogdan after unlocking the vault.'));
...
(function(){
  ...
  function refreshJoeyBundleFromRedis(opts){
    ...
    joeyBundleRefreshInflight = joeyFetchJson(joeyActionUrl('bundle', pass, activeMode), { cache:'no-store' }).then(function(data){
  ...
  function maybeRefreshJoeyBundleFromRedis(opts){
    ...
    joeySyncMetaInflight = joeyFetchJson(joeyActionUrl('sync-meta', pass, activeMode), { cache:'no-store' }).then(function(data){
