...
document.addEventListener('DOMContentLoaded', function(){
      ...
      const elToday=document.getElementById('pom-today');
      const elNotify=document.getElementById('pom-notify');
      const elAuto=document.getElementById('pom-auto');
      ...
      function loadToday(){ const d=loadJSON(DKEY,{date:'',mins:0}); const today=new Date().toISOString().slice(0,10); return d.date===today?d:{date:today,mins:0}; }
      function addFocusTime(mins){ const d=loadToday(); d.mins+=mins; saveJSON(DKEY,d); updateMeta(); }
      // FIX: Cycle math correctly computes "1/4" when pomodoros is 0
      function updateMeta(){
        elCount.textContent=state.pomodoros;
        elCycle.textContent=(state.pomodoros % settings.longEvery) + 1;
      ...
      function playHomerVoice(key) {
        if(!elVoice || !elVoice.checked) return false;
        if(!homerBuf[key] || !ensureAC()) return false;
        var src = ac.createBufferSource();
        var g = ac.createGain();
        src.buffer = homerBuf[key];
        g.gain.value = 0.9;
      ...
      function sfxWooHoo(){ if(playBuf('woohoo',0.7) || playHomerVoice('woohoo')) return; speak("Woo-hoo!"); tone(880,0.12,'triangle',0.25,0); tone(1046.5,0.12,'triangle',0.22,0.10); tone(1318.5,0.18,'triangle',0.22,0.22); }
      function sfxDoh(){ if(playBuf('doh',0.7) || playHomerVoice('doh')) return; speak("D'oh!"); tone(196,0.18,'sawtooth',0.25,0); tone(147,0.18,'sawtooth',0.20,0.10); }
      ...
      function notify(title,body){
        ...
        var sanitize = typeof window._homerFixMojibakeText === 'function'
          ? window._homerFixMojibakeText
          : function(value){ return String(value || ''); };
        var safeTitle = sanitize(String(title || ''));
        var safeBody = sanitize(String(body || ''));
        if('Notification' in window && Notification.permission==='granted'){
          const n=new Notification(safeTitle,{ body:safeBody }); setTimeout(()=>n.close(),4000);
        }
      }
      ...
      document.addEventListener('keydown',function(e){
        ...
        const pomTab=document.getElementById('tab-pomodoro');
        if(!pomTab||pomTab.style.display==='none') return;
...
function setSoundBtnState(btn, playing) {
  if(!btn) return;
  if(playing){ btn.classList.add('playing'); btn.innerHTML = '<span>&#9646;&#9646;</span> Pause'; }
  else { btn.classList.remove('playing'); btn.innerHTML = '<span>&#9654;</span> Play'; }
}
...
(function(){
  ...
  // Label definitions
  var DEFAULT_LABELS = [
    {name:'work', color:'#FF3380', bg:'rgba(59,130,246,.15)'},
    {name:'personal', color:'#8b5cf6', bg:'rgba(139,92,246,.15)'},
    {name:'health', color:'#CC00FF', bg:'rgba(204,0,255,.15)'},
  ...
  var DEFAULT_PROJECTS = [
    {id:'organize-life', key:'ORG', name:'OrganizeLife', description:'Household systems, routines, planning, errands, and personal operating tasks.', color:'#FF0066', icon:'OL', order:0},
    {id:'apps', key:'APP', name:'Apps', description:'Product ideas, app builds, launches, fixes, and feature delivery.', color:'#CC00FF', icon:'AP', order:1}
  ];
  ...
  function ensureProject(project, index){
    ...
    var normalized = {
      ...
      color: /^#[0-9a-f]{6}$/i.test(raw.color || '') ? raw.color : '#FF0066',
  ...
  function renderCustomFieldInputsHtml(project, values, classPrefix){
    ...
    var controlStyle = 'width:100%;padding:9px 10px;border-radius:10px;border:1px solid rgba(160,150,200,.14);background:rgba(13,0,32,.42);color:var(--text);font-size:.84rem;';
    return fields.map(function(field){
      ...
      if(field.type === 'select'){
        ...
        inputHtml = '<label style="display:flex;align-items:center;gap:8px;padding:9px 10px;border-radius:10px;border:1px solid rgba(160,150,200,.14);background:rgba(13,0,32,.42);"><input type="checkbox" class="' + classPrefix + '-input" data-field-id="' + field.id + '"' + (value ? ' checked' : '') + '><span>' + escHtml(customFieldLabel(field)) + '</span></label>';
      } else {
      ...
      return '<div class="project-custom-field" data-field-type="' + field.type + '" style="display:flex;flex-direction:column;gap:5px;">' +
        (field.type === 'toggle' ? '' : '<label style="font-size:.72rem;font-weight:700;color:rgba(160,150,200,.88);">' + escHtml(customFieldLabel(field)) + '</label>') +
        inputHtml +
      '</div>';
    }).join('');
  }
  ...
  function loadVaultRoot(){
    ...
    return idb.get(DATA_KEY).then(function(stored){
      ...
      return decrypt(cryptoKey, stored).then(function(json){
        ...
        _vaultRootCache = defaultVaultRoot();
        return _vaultRootCache;
      });
    });
  }
  ...
  function saveVaultForMode(mode, data){
    if(!cryptoKey) return Promise.reject(new Error('Vault is locked'));
    return loadVaultRoot().then(function(root){
  ...
  function createProjectEntry(projectData, options){
    ...
    return loadVaultForMode(mode).then(function(data){
      ...
      data.projects = ensureProjects((data.projects || []).concat([{
        ...
        color: /^#[0-9a-f]{6}$/i.test(projectData.color || '') ? projectData.color : '#FF0066',
  ...
  function updateProjectEntry(projectId, projectData, options){
    ...
    return loadVaultForMode(mode).then(function(data){
      ...
      project.color = /^#[0-9a-f]{6}$/i.test(projectData && projectData.color || '') ? projectData.color : (project.color || '#FF0066');
  ...
  function renderProjectDashboard(data){
    ...
    html += statCard(summary.progress, '#FF0066', 'Delivery', summary.progress + '%', 'average issue progress');
    html += statCard(completedPct, '#CC00FF', 'Completion', summary.counts.done + '/' + summary.total, 'issues closed');
  ...
  function subtaskProgress(subs){
    ...
    var color = pct === 100 ? '#CC00FF' : 'var(--accent)';
  ...
  function openIssueFullscreen(goalIdx, subIdx){
    ...
    loadVault().then(function(data){
      ...
      if(isSub){
        ...
        html += '<select class="issue-edit-sub-effort" style="padding:4px 8px;border-radius:6px;border:1px solid rgba(255,255,255,.1);background:#100020;color:var(--text);font-size:.82rem;font-weight:600;cursor:pointer;">';
        ...
        html += '<input type="date" class="issue-edit-sub-due" value="'+(sub.due||'')+'" style="padding:4px 8px;border-radius:6px;border:1px solid rgba(255,255,255,.1);background:#100020;color:var(--text);font-size:.82rem;font-weight:600;cursor:pointer;max-width:150px;">';
        html += '</span></div>';
        html += '</div>';
        ...
        html += '<select class="issue-edit-project" style="padding:4px 8px;border-radius:6px;border:1px solid rgba(255,255,255,.1);background:#100020;color:var(--text);font-size:.82rem;font-weight:600;cursor:pointer;max-width:160px;">';
        ...
        html += '<select class="issue-edit-priority" style="padding:4px 8px;border-radius:6px;border:1px solid rgba(255,255,255,.1);background:#100020;color:var(--text);font-size:.82rem;font-weight:600;cursor:pointer;">';
        ...
        html += '<div class="issue-field-row"><span class="issue-field-label">Due Date</span><span class="issue-field-value">';
        html += '<input type="date" class="issue-edit-due" value="'+(g.due||'')+'" style="padding:4px 8px;border-radius:6px;border:1px solid rgba(255,255,255,.1);background:#100020;color:var(--text);font-size:.82rem;font-weight:600;cursor:pointer;max-width:150px;">';
        html += '</span></div>';
        ...
        html += '<input type="text" class="issue-edit-reporter" value="'+escAttr(g.reporter||'')+'" placeholder="—" style="padding:4px 8px;border-radius:6px;border:1px solid rgba(255,255,255,.1);background:#100020;color:var(--text);font-size:.82rem;font-weight:600;width:100%;max-width:150px;">';
        html += '</span></div>';
        ...
        html += '<input type="text" class="issue-edit-assignee" value="'+escAttr(g.assignee||'')+'" placeholder="—" style="padding:4px 8px;border-radius:6px;border:1px solid rgba(255,255,255,.1);background:#100020;color:var(--text);font-size:.82rem;font-weight:600;width:100%;max-width:150px;">';
        html += '</span></div>';
        html += '</div>';
        ...
        html += '</div>';
        html += '<div style="display:flex;gap:4px;"><input type="text" class="issue-label-new" placeholder="New label..." style="flex:1;padding:4px 8px;border-radius:6px;border:1px solid rgba(255,255,255,.1);background:#100020;color:var(--text);font-size:.75rem;"><button class="issue-label-add" style="padding:4px 10px;border-radius:6px;border:1px solid rgba(255,0,102,.25);background:none;color:#FF0066;cursor:pointer;font-size:.7rem;font-weight:700;">+</button></div>';
        html += '</div>';
        ...
        ['todo','progress','pending','backlog','done'].forEach(function(col){
          ...
          html += '<button class="issue-move-btn" data-to="'+col+'" style="padding:6px 12px;border-radius:8px;border:1px solid '+(active?'var(--accent)':'rgba(255,255,255,.15)')+';background:'+(active?'rgba(255,0,102,.1)':'none')+';color:'+(active?'var(--accent)':'var(--muted)')+';cursor:pointer;font-size:.78rem;font-weight:700;transition:all .15s;">'+colLabels[col]+'</button>';
        });
        html += '</div></div>';
  ...
  function _renderGoalsNow(goals, projects){
    ...
    filtered.forEach(function(g, fi){
      ...
      if(g.col==='done' && g.completedAt) statusBadge = '<span style="font-size:.6rem;padding:1px 6px;border-radius:5px;background:rgba(204,0,255,.15);color:#CC00FF;font-weight:700;">Done '+new Date(g.completedAt).toLocaleDateString('en-GB',{day:'numeric',month:'short'})+'</span>';
      if(g.col==='backlog') statusBadge = '<span style="font-size:.6rem;padding:1px 6px;border-radius:5px;background:rgba(160,150,200,.16);color:#d0c0f0;font-weight:700;">BACKLOG</span>';
      ...
      card.innerHTML = titleRow + labelsHtml + peopleHtml + collapsedExtra +
        '<div class="k-body">' +
          '<input class="k-field k-summary" value="'+escAttr(g.summary||'')+'" placeholder="Summary" style="width:100%;padding:8px 10px;border-radius:8px;border:1px solid rgba(255,255,255,.1);background:#100020;color:var(--text);font-size:.95rem;font-weight:700;margin-bottom:8px;">' +
          '<textarea class="k-field k-notes" placeholder="Description..." rows="3">'+escHtml(g.notes||'')+'</textarea>' +
          labelEditorHtml +
          '<div style="display:flex;gap:6px;margin-top:8px;">' +
            '<div style="flex:1;"><label style="font-size:.68rem;color:var(--muted);">Reporter</label><input class="k-field k-reporter" value="'+escAttr(g.reporter||'')+'" placeholder="Reporter" style="width:100%;padding:6px 8px;border-radius:6px;border:1px solid rgba(255,255,255,.1);background:#100020;color:var(--text);font-size:.8rem;margin-top:2px;"></div>' +
            '<div style="flex:1;"><label style="font-size:.68rem;color:var(--muted);">Assignee</label><input class="k-field k-assignee" value="'+escAttr(g.assignee||'')+'" placeholder="Assignee" style="width:100%;padding:6px 8px;border-radius:6px;border:1px solid rgba(255,255,255,.1);background:#100020;color:var(--text);font-size:.8rem;margin-top:2px;"></div>' +
          '</div>' +
          (project && project.customFields && project.customFields.length
            ? '<div style="margin-top:10px;"><div style="font-size:.72rem;font-weight:800;letter-spacing:.04em;text-transform:uppercase;color:rgba(160,150,200,.88);margin-bottom:6px;">Project Fields</div><div class="k-custom-fields" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:8px;">' + renderCustomFieldInputsHtml(project, g.customFields || {}, 'k-cf') + '</div></div>'
            : '') +
  ...
  function openProjectModal(projectId){
    ...
    if(projectId){
      loadVault().then(function(data){
        ...
        projectColorInput.value = displayProject.color || project.color || '#FF0066';
    ...
    projectColorInput.value = '#FF0066';
    projectModalBg.classList.add('open');
    setTimeout(function(){ projectNameInput.focus(); }, 80);
  }
  ...
  function renderProjectFieldRows(fields){
    ...
    if(!list.length){
      projectFieldsList.innerHTML = '<div style="padding:12px;border-radius:12px;border:1px dashed rgba(160,150,200,.2);background:rgba(13,0,32,.42);color:var(--muted);font-size:.82rem;">No custom fields yet. Add status-like metadata such as squad, estimate, target release, customer, or QA signoff.</div>';
      return;
    }
    projectFieldsList.innerHTML = list.map(function(field){
      var optionsValue = (field.options || []).join(', ');
      return '<div class="project-field-row" data-field-id="' + field.id + '" style="padding:12px;border-radius:14px;border:1px solid rgba(160,150,200,.18);background:rgba(13,0,32,.48);display:grid;grid-template-columns:' + (compact ? '1fr' : '1.5fr .9fr 1.4fr auto') + ';gap:8px;align-items:center;">' +
        '<input class="project-field-name" type="text" value="' + escAttr(field.name) + '" placeholder="Field name" style="width:100%;padding:9px 10px;border-radius:10px;border:1px solid rgba(160,150,200,.14);background:rgba(8,0,26,.55);color:var(--text);">' +
        '<select class="project-field-type" style="width:100%;padding:9px 10px;border-radius:10px;border:1px solid rgba(160,150,200,.14);background:rgba(8,0,26,.55);color:var(--text);">' +
        ...
        '</select>' +
        '<input class="project-field-options" type="text" value="' + escAttr(optionsValue) + '" placeholder="Options, comma separated" style="width:100%;padding:9px 10px;border-radius:10px;border:1px solid rgba(160,150,200,.14);background:rgba(8,0,26,.55);color:var(--text);' + (field.type === 'select' ? '' : 'opacity:.55;') + '"' + (field.type === 'select' ? '' : ' disabled') + '>' +
  ...
  if(projectSubmitBtn) projectSubmitBtn.addEventListener('click', function(){
    ...
    var projectPayload = {
      ...
      color: projectColorInput.value || '#FF0066'
    };
  ...
  function decodeHexSecret(parts){
    var hex = String((parts || []).join(''));
    var out = '';
  ...
  function getReservedVaultLegacyPasswords(user){
    if(String(user || '').trim().toLowerCase() !== 'bogdan') return [];
    return [decodeHexSecret(['51415a77', '73783039', '38706c2e', '21'])];
  }
  function tryLoadVaultRootWithKey(key){
    return idb.get(DATA_KEY).then(function(stored){
      if(!stored) return { key:key, root:defaultVaultRoot(), hasStored:false };
      return decrypt(key, stored).then(function(json){
        return { key:key, root:ensureVaultRoot(JSON.parse(json)), hasStored:true };
      });
    });
  }
  function deriveVaultSession(user, pass, salt){
    var vaultPw = user.toLowerCase() + ':' + pass;
    return deriveKey(vaultPw, salt).then(function(key){
      if(isNew) return { key:key, root:defaultVaultRoot(), hasStored:false, vaultPw:vaultPw };
      return tryLoadVaultRootWithKey(key).then(function(result){
        result.vaultPw = vaultPw;
        return result;
      });
    });
  }
  function rekeyVaultRoot(root, user, nextPass, salt){
    var normalizedRoot = ensureVaultRoot(root);
    var nextVaultPw = user.toLowerCase() + ':' + nextPass;
    return hashPassword(nextVaultPw, salt).then(function(nextHash){
      return deriveKey(nextVaultPw, salt).then(function(nextKey){
        return encrypt(nextKey, JSON.stringify(normalizedRoot)).then(function(enc){
          return idb.set(DATA_KEY, enc).then(function(){
            return idb.set(HASH_KEY, nextHash);
          }).then(function(){
            _vaultRootCache = normalizedRoot;
            cryptoKey = nextKey;
            return nextKey;
          });
        });
...
(function(){
  ...
  var CAL_CATEGORIES = {
    ...
    work:     {label:'Work',     color:'#FF3380', bg:'rgba(59,130,246,.15)'}
  };
  ...
  function pColor(p){
    ...
    if(p==='low') return '#CC00FF';
    if(p==='ext') return '#FF3380';
  ...
  var _labelPalette = ['#ef4444','#f97316','#f59e0b','#CC00FF','#14b8a6','#0ea5e9','#FF3380','#6366f1','#8b5cf6','#a855f7','#ec4899','#f43f5e'];
  ...
  function heartbeatNotify(title, body){
    var sanitize = typeof window._homerFixMojibakeText === 'function'
      ? window._homerFixMojibakeText
      : function(value){ return String(value || ''); };
    var safeTitle = sanitize(String(title || 'Reminder')).trim() || 'Reminder';
    var safeBody = sanitize(String(body || '')).trim();
    if('Notification' in window && Notification.permission === 'granted'){
      try{
  ...
  function showDetail(item, refEl){
    ...
    if(item.status){
      var statusColors = {CONFIRMED:'#CC00FF',TENTATIVE:'#fbbf24',CANCELLED:'#ef4444'};
    ...
    // URL
    if(item.url){
      h += '<div style="margin-top:8px;"><a href="' + esc(item.url) + '" target="_blank" style="color:#FF0066;font-size:.78rem;text-decoration:none;word-break:break-all;">&#128279; Open link</a></div>';
    }
  ...
  showPop = function(dayEl, items, key){
    ...
    items.forEach(function(it, idx){
      ...
      var dotColor = it.type === 'outlook' ? '#FF3380' : hasCat ? CAL_CATEGORIES[it.category].color : it.type === 'custom' ? '#a78bfa' : pColor(it.priority);
  ...
  function renderDayView(dueMap){
    ...
    calDayTitle.style.color = isToday ? '#FF0066' : 'var(--text)';
    ...
    function renderAgendaItem(it){
      ...
      if(it.icsPriority && it.icsPriority > 0){
        ...
        h += '<span style="color:' + (it.icsPriority <= 4 ? '#ef4444' : it.icsPriority === 5 ? '#fbbf24' : '#CC00FF') + '">&#9650; ' + pLabel + '</span>';
      }
...
(function(){
  ...
  // Migrate: if homer-auth-user was previously stored as the Supabase email, normalize to the username
  (function(){
    var stored = localStorage.getItem(AUTH_USER_KEY);
    if(stored && stored.toLowerCase() === 'bogdan.radu@b4it.ro'){
      localStorage.setItem(AUTH_USER_KEY, 'bogdan');
    }
  })();
  ...
  function isReservedSyncUser(user){
    return String(user || '').trim().toLowerCase() === RESERVED_SYNC_USER;
  }
  function setAuthState(user, hash, permissions){
    localStorage.setItem(AUTH_USER_KEY, user);
    localStorage.setItem(AUTH_HASH_KEY, hash);
    if(permissions){
      localStorage.setItem('homer-user-permissions', JSON.stringify(permissions));
    } else {
  ...
  function restoreCloudSignin(data, fallbackUser, hash, permissions){
      ...
      if(idbK.indexOf(k) >= 0){
        ...
        idbW.push(new Promise(function(res){
          iReq.onsuccess = function(ev){
            ...
            tx.objectStore('kv').put(data[k], k);
            tx.oncomplete = res;
            tx.onerror = res;
          };
          iReq.onerror = res;
  ...
  function verifyReservedAccount(user, pass){
    ...
    var basePayload = { ok: true, username: user, email: email, permissions: { vault: true, joey: true }, session: null, userData: null };
    function tryHashFallback(){
      return hashCreds(user, pass).then(function(hash){
        return fetch('/api/auth', {
          method: 'POST',
          headers: {'Content-Type':'application/json'},
          body: JSON.stringify({action:'verifyHash', hash: hash})
        }).then(function(r){ return r.json(); }).then(function(d){
          if(d && d.ok) return basePayload;
  ...
  function fetchSyncData(syncHash){
    return fetch(R2_WORKER_URL + '/sync', {headers:{'X-Sync-Key':syncHash}})
      .then(function(r){ return r.json(); })
      .catch(function(){ return null; });
  }
  function tryReservedCloudRestore(user, hashes, index){
    if(index >= hashes.length) return Promise.resolve(null);
    return fetchSyncData(hashes[index]).then(function(d){
      if(d && d.data && d.data[AUTH_USER_KEY] && d.data[AUTH_USER_KEY].toLowerCase() === user.toLowerCase()){
        return d.data;
      }
      return tryReservedCloudRestore(user, hashes, index + 1);
    });
  }
  function applySharedSyncIdentity(user, hash){
    if(isReservedSyncUser(user) && hash){
  ...
  submitBtn.addEventListener('click', async function(){
    ...
    if(isRegister){
      ...
      if(isReservedSyncUser(user)){ errorEl.textContent = 'The username "bogdan" is reserved.'; return; }
      localStorage.setItem(AUTH_USER_KEY, user);
      localStorage.setItem(AUTH_HASH_KEY, hash);
      localStorage.removeItem('homer-user-permissions');
      applySharedSyncIdentity(user, hash);
      ...
      if(isReservedSyncUser(user)){
        ...
        verifyReservedAccount(user, pass).then(function(d){
          ...
          if(storedUser && storedUser.toLowerCase() === verifiedUser.toLowerCase()){
            finalizeSignin(verifiedUser, hash, permissions);
            return;
          }
          var syncHashes = [hash].concat(RESERVED_SYNC_LEGACY_HASHES.filter(function(item){ return item !== hash; }));
          return tryReservedCloudRestore(verifiedUser, syncHashes, 0).then(function(cloudData){
            if(cloudData) return restoreCloudSignin(cloudData, verifiedUser, hash, permissions);
            finalizeSignin(verifiedUser, hash, permissions);
          });
        }).catch(function(err){
          errorEl.textContent = err && err.message ? err.message : 'Invalid username or password';
          submitBtn.disabled = false;
          submitBtn.textContent = 'Sign In';
        });
        return;
      ...
      if(storedUser && storedHash){
        ...
        if(isReservedSyncUser(storedUser)){
...
(function(){
  ...
  if(!toggleBtn || !loginSection || !panel || !userInput || !passInput || !signinBtn || !errorEl || !refreshBtn || !listEl || !statusEl){
    return;
  }
        ...
        card.innerHTML =
          '<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;">' +
            ...
            '<div style="display:flex;align-items:center;gap:6px;flex-shrink:0;">' +
              ...
              '<button class="admin-restore-btn" data-key="' + escH(e.key) + '" data-name="' + escH(name) + '" style="background:none;border:1px solid rgba(204,0,255,.25);border-radius:6px;padding:3px 8px;color:#CC00FF;cursor:pointer;font-size:.7rem;font-weight:700;">Restore</button>' +
              '<button class="admin-inspect-btn" data-key="' + escH(e.key) + '" style="background:none;border:1px solid rgba(255,0,102,.25);border-radius:6px;padding:3px 8px;color:#FF0066;cursor:pointer;font-size:.7rem;font-weight:700;">View</button>' +
  ...
  function switchAdminTab(tab){
    if(tab === 'backups'){
      ...
      backupsTab.style.background = 'linear-gradient(135deg,var(--accent),#9900CC)';
      ...
      usersTab.style.background = 'linear-gradient(135deg,var(--accent),#9900CC)';
      ...
      d.users.forEach(function(u){
        ...
        card.innerHTML =
          '<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;">' +
            ...
            '</div>' +
            '<div style="display:flex;align-items:center;gap:6px;flex-shrink:0;">' +
              '<button class="admin-user-edit-btn" data-user="' + escH(u.username) + '" style="background:none;border:1px solid rgba(255,0,102,.25);border-radius:6px;padding:3px 8px;color:#FF0066;cursor:pointer;font-size:.7rem;font-weight:700;">Edit</button>' +
...
(function(){
  ...
  var LS_FIELD_MAP = {
    ...
    'homer-expense-budgets-excluded': 'ls:homer-expense-budgets-excluded',
  ...
  var SHARED_SYNC_USER = 'bogdan';
  function getSharedAuthUser(){
    return String(localStorage.getItem('homer-auth-user') || '').trim().toLowerCase();
  }
  ...
  window._homerFetchJson = fetchJson;
  ...
  function getSyncJwt(){
    try{
      ...
      var sbSess = window.__sbSession;
      if(sbSess && sbSess.access_token) return 'Bearer ' + sbSess.access_token;
    }catch(_e){}
    return '';
  }
  function isFieldSyncReady(){
    return !!(String(localStorage.getItem(SYNC_PASS_KEY) || pass.value || '').trim() || getSyncJwt());
  }
  ...
  function flushFieldSyncOps(reason){
    ...
    var jwt = getSyncJwt();
    var p = jwt ? '' : String(localStorage.getItem(SYNC_PASS_KEY) || pass.value || '').trim();
    if(!jwt && !p) return Promise.resolve({ ok:false, skipped:true, reason:'nologin' });
    ...
    var bodyData = { action:'field-op', deviceId:getFieldSyncDeviceId(), reason:reason||'field-sync', ops:batch };
    if(p) bodyData.key = p;
  ...
  function hydrateFieldSyncState(force){
    ...
    var url = DB_BACKUP_URL + '?action=' + encodeURIComponent(action);
    if(p) url += '&key=' + encodeURIComponent(p);
    if(action === 'field-ops') url += '&since=' + encodeURIComponent(String(since)) + '&limit=120';
    var fetchOpts = { cache:'no-store' };
    if(jwt) fetchOpts.headers = { 'Authorization': jwt };
  ...
  function getPersistentFieldSyncState(el){
    ...
    if(!jwt && !syncPass){
      return { tone:'idle', text:'Sync off', title:'Sign in to enable field sync.' };
    }
    var fieldId = el && el.dataset && el.dataset.syncedFieldId;
    if(fieldId && fieldSyncPendingMap && fieldSyncPendingMap.has(fieldId)){
      return { tone:'pending', text:'Pending', title:'Field sync pending.' };
    }
    if(jwt){
      return { tone:'synced', text:'Synced', title:'Field synced via Supabase Realtime.' };
    }
    return { tone:'idle', text:'No sync', title:'Field not yet synced.' };
  }
  ...
  function renderHealthCard(label, ts, detail, accent){
    var tone = accent || '#FF0066';
    return '<div style="padding:10px 12px;border-radius:12px;border:1px solid rgba(160,150,200,.18);background:linear-gradient(180deg,rgba(13,0,32,.88),rgba(8,0,26,.92));min-height:78px;">' +
  ...
  function renderBackupHealth(){
    ...
    backupHealthEl.innerHTML =
      ...
      renderHealthCard('Cloud Snapshot', localStorage.getItem('homer-backup-ts'), backupCount + ' keys stored in cloud snapshot. Source: ' + source + '.', '#FF0066') +
      ...
      renderHealthCard('Emergency Drive Snapshot', localStorage.getItem(BACKUP_EMERGENCY_DRIVE_TS_KEY), 'Standalone whole-app snapshot for emergency recovery.', '#CC00FF') +
  ...
  function openBackupInspector(title, snapshot, meta){
    ...
    overlay.style.cssText = 'position:fixed;inset:0;z-index:10040;background:rgba(8,0,26,.78);display:flex;align-items:center;justify-content:center;padding:18px;';
    var panel = document.createElement('div');
    panel.style.cssText = 'width:min(1080px,96vw);max-height:88vh;overflow:auto;border-radius:18px;border:1px solid rgba(160,150,200,.24);background:#07111f;box-shadow:0 25px 70px rgba(0,0,0,.45);padding:18px;';
    var lines = [
    ...
    panel.innerHTML =
      '<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:12px;">' +
        '<div><div style="font-size:1rem;font-weight:800;color:#f8f0ff;">' + safeEscHtml(title) + '</div><div style="font-size:.78rem;color:var(--muted);margin-top:4px;">' + lines.join(' | ') + '</div></div>' +
        '<button type="button" style="padding:8px 12px;border-radius:10px;border:none;background:#180030;color:#fff;cursor:pointer;">Close</button>' +
      '</div>' +
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">' +
        '<div style="padding:12px;border-radius:14px;border:1px solid rgba(160,150,200,.18);background:rgba(13,0,32,.72);">' +
          '<div style="font-size:.72rem;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:#FF88AA;margin-bottom:8px;">Manifest</div>' +
        ...
        '<div style="padding:12px;border-radius:14px;border:1px solid rgba(160,150,200,.18);background:rgba(13,0,32,.72);">' +
          ...
          '<pre style="margin:0;white-space:pre-wrap;word-break:break-word;color:#ece0ff;font-size:.78rem;line-height:1.5;">' + safeEscHtml(JSON.stringify(integrity, null, 2)) + '</pre>' +
        '</div>' +
      '</div>' +
      '<div style="margin-top:12px;padding:12px;border-radius:14px;border:1px solid rgba(160,150,200,.18);background:rgba(13,0,32,.72);">' +
        ...
        '<pre style="margin:0;white-space:pre-wrap;word-break:break-word;color:#ece0ff;font-size:.78rem;line-height:1.5;">' + safeEscHtml(keys.join('\n')) + '</pre>' +
        '</div>' +
      '';
  ...
  function ensureVaultServerMirror(){
    // Field state is now synced via Supabase Realtime — no Redis mirror needed
  }
  var lastBackupHash = '';
  ...
  function doDbBackup(p, allData, opts){
    // Redis full-snapshot mirror removed — field state synced via Supabase Realtime
    return Promise.resolve({ ok:true, skipped:true });
  }

  function scheduleDbBackup(reason){
    // Redis mirror removed — no-op
  }
  ...
  function backup(force){
    ...
    if(!p){ st('No sync passphrase set. Open Account and sign in first.'); alert('No sync passphrase set. Please sign in via the Account button first.'); return Promise.resolve({ ok:false, skipped:true, reason:'nologin' }); }
    ...
    fetch(R2_WORKER_URL+'/versions',{headers:{'X-Sync-Key':p}})
      ...
      .then(function(d){
        ...
        versions.slice().reverse().forEach(function(v, i){
          ...
          h += '<button class="fab-restore-version" data-ts="' + v.ts + '" style="background:none;border:1px solid rgba(255,0,102,.25);border-radius:6px;padding:2px 8px;color:#FF0066;cursor:pointer;font-size:.68rem;font-weight:700;">Restore</button>';
          h += '</div>';
        });
  ...
  function restore(){
    var p = getActiveSyncPass();
    if(!p){ st('Shared restore is available only for Bogdan after unlocking the vault.'); return Promise.resolve({ ok:false, skipped:true, reason:'nologin' }); }
    localStorage.setItem(SYNC_PASS_KEY, p);
    ...
    fetchJson(R2_WORKER_URL + '/versions', { headers:{'X-Sync-Key':p}, cache:'no-store' })
      .then(function(d){
        ...
        versions.slice().reverse().forEach(function(v){
          ...
          h += '<button class="fab-inspect-version" data-ts="' + v.ts + '" style="background:none;border:1px solid rgba(160,150,200,.22);border-radius:6px;padding:2px 8px;color:#d0c0f0;cursor:pointer;font-size:.68rem;font-weight:700;">Inspect</button>';
          h += '<button class="fab-restore-version" data-ts="' + v.ts + '" style="background:none;border:1px solid rgba(255,0,102,.25);border-radius:6px;padding:2px 8px;color:#FF0066;cursor:pointer;font-size:.68rem;font-weight:700;">Restore</button>';
          h += '</div>';
          h += '</div>';
        });
  ...
  if(inspectLatestBtn){
    inspectLatestBtn.addEventListener('click', function(){
      var p = getActiveSyncPass();
      if(!p){ st('Shared sync is available only for Bogdan after unlocking the vault.'); return; }
  ...
  autoChk.addEventListener('change', function(){
    if(autoChk.checked && !isSharedSyncUser()){
      autoChk.checked = false;
      st('Shared sync is reserved for Bogdan. Other accounts stay local-only.');
      updDot();
      return;
    }
  ...
  function startAuto(){
    ...
    _pullPromise.then(function(){
      ...
      if(getActiveSyncPass() && dirty){ autoBackup(); }
    });
  }
  function stopAuto(){
  ...
  function tryLead(triggerSync){
    ...
    if(!cur || cur === TAB_ID || !leaderAlive()){
      claimLeader();
      if(triggerSync && getActiveSyncPass() && localStorage.getItem(SYNC_AUTO_KEY) === 'true'){
  ...
  function flushDbBackupNow(reason){
    // Redis mirror removed — field state synced via Supabase Realtime
  ...
  // ── Supabase Realtime: apply incoming field changes ──────────
  // When Supabase pushes a field_state change, apply it directly —
  // no polling needed. Ignore changes from the same device.
...
(function(){
  ...
  function hasJoeyRemoteAuth(){
    return !!(String(localStorage.getItem('homer-sync-pass') || '').trim() || getSupabaseAuthHeaderValue());
  }
  function withSupabaseAuth(options){
    var source = options && typeof options === 'object' ? options : {};
    var next = {};
    Object.keys(source).forEach(function(key){ next[key] = source[key]; });
    var authHeader = getSupabaseAuthHeaderValue();
    if(!authHeader) return next;
    var headers = {};
    var sourceHeaders = source.headers && typeof source.headers === 'object' ? source.headers : {};
    Object.keys(sourceHeaders).forEach(function(key){ headers[key] = sourceHeaders[key]; });
  ...
  function refreshJoeySyncStatus(opts){
    ...
    var pass = localStorage.getItem('homer-sync-pass') || '';
    if(!hasJoeyRemoteAuth()){
      renderJoeySyncStatusState('Sign in or unlock Joey memory to check sync status.', [], []);
      return Promise.resolve(null);
    }
  ...
  function shouldPreferBuiltInSpeechRecognition(){
    if(!SpeechRecognitionCtor) return false;
    if(isAndroidDevice()) return true;
    return !isIosVoiceFallbackPreferred();
  }
  ...
  function syncVoiceUi(){
    ...
    voiceBtn.title = !SpeechRecognitionCtor
      ? 'Use keyboard microphone'
      : (busy ? 'Processing voice command' : (listening ? 'Stop voice input' : 'Start voice input'));
  }
  ...
  function startVoiceInput(){
    if(!SpeechRecognitionCtor){
      ...
      setUploadStatus('Built-in speech is not available here. Use your keyboard microphone in the text box.', 'warn', true);
      syncVoiceUi();
      return;
    }
    if(streaming){
  ...
  function fetchFileLibrary(opts){
    ...
    var pass = localStorage.getItem('homer-sync-pass') || '';
    return fetch(joeyActionUrl('file-library', pass), withSupabaseAuth({ cache:'no-store' }))
  ...
  function scheduleJoeyLearnRetry(reason, delayMs){
    if(!hasJoeyRemoteAuth() || joeyAutoLearnRetryTimer) return;
  ...
  function commitJoeyMemory(opts){
    ...
    var pass = localStorage.getItem('homer-sync-pass') || '';
    if(!hasJoeyRemoteAuth()){
      if(!opts.silent) alert('Sign in or unlock Joey memory to commit memory.');
      return Promise.reject(new Error('Missing sync credentials'));
    }
    ...
    return collectTaskSnapshot().then(function(tasks){
      return fetch('/api/joey?action=commit', withSupabaseAuth({
        method:'POST',
        headers:{'Content-Type':'application/json'},
  ...
  // --- Google Drive context restore (two-phase to avoid CF 30s wall limit) ---
  ...
  if(gdriveRestoreBtn) gdriveRestoreBtn.addEventListener('click', function(){
    var btn = this;
    var pass = localStorage.getItem('homer-sync-pass') || '';
    if(!hasJoeyRemoteAuth()){ alert('Log in first to restore from Drive.'); return; }
    ...
    btn.disabled = true;
    btn.textContent = '⏳ Fetching from Drive...';
    // Phase 1: fetch from Drive only (no DB writes — fits under CF 30s wall limit)
    window._homerFetchJson('/api/gdrive-restore', withSupabaseAuth({
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body: JSON.stringify(withContextMode({ passphrase: pass, kind: 'drive-fetch' }))
    })).then(function(fd){
      if(!fd || !fd.ok || !fd.driveData) {
        btn.textContent = '❌ Failed';
        console.error('[GDrive Restore] Phase 1:', fd);
        alert('Drive fetch error:\n' + JSON.stringify(fd, null, 2).slice(0, 500));
        setTimeout(function(){ btn.innerHTML = '&#x21bb; Restore from Drive'; btn.disabled = false; }, 2000);
        return;
      }
      btn.textContent = '⏳ Applying to DB...';
      // Phase 2: apply fetched data to DB (no Drive fetch — fits under CF 30s wall limit)
      return window._homerFetchJson('/api/gdrive-restore', withSupabaseAuth({
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body: JSON.stringify(withContextMode({ passphrase: pass, kind: 'drive-apply', driveData: fd.driveData }))
      })).then(function(d){
        if(d.ok){
          ...
          btn.textContent = '❌ Failed';
          console.error('[GDrive Restore] Phase 2:', d);
  ...
  var joeyBundleRefreshInflight = null;
  var joeyBundleRefreshInflightMode = '';
  var joeySyncMetaInflight = null;
  var joeySyncMetaInflightMode = '';
  ...
  function getJoeyModeKey(mode){
    return mode === 'work' ? 'work' : 'personal';
  }
  function getJoeyHistorySaveSignature(mode, messages){
    var activeMode = getJoeyModeKey(mode);
    var normalized = Array.isArray(messages) ? messages.map(function(item){
      return {
        role: item && item.role === 'assistant' ? 'assistant' : 'user',
        content: String(item && item.content || '')
      };
    }) : [];
    return JSON.stringify({ mode:activeMode, messages:normalized });
  }
  ...
  function joeyFetchJson(url, options){
    var nextOptions = withSupabaseAuth(options);
    if(typeof window._homerFetchJson === 'function') return window._homerFetchJson(url, nextOptions);
  ...
  function refreshJoeyBundleFromRedis(opts){
    opts = opts || {};
    var activeMode = getJoeyModeKey(opts.mode || currentContextMode);
    var pass = localStorage.getItem('homer-sync-pass') || '';
    if(!hasJoeyRemoteAuth()) return Promise.reject(new Error('Missing sync credentials'));
    if(joeyBundleRefreshInflight && joeyBundleRefreshInflightMode === activeMode) return joeyBundleRefreshInflight;
    joeyBundleRefreshInflightMode = activeMode;
    joeyBundleRefreshInflight = joeyFetchJson(joeyActionUrl('bundle', pass, activeMode), withSupabaseAuth({ cache:'no-store' })).then(function(data){
      ...
      joeyBundleRefreshInflight = null;
      joeyBundleRefreshInflightMode = '';
    });
    return joeyBundleRefreshInflight;
  }
  function maybeRefreshJoeyBundleFromRedis(opts){
    opts = opts || {};
    var activeMode = getJoeyModeKey(opts.mode || currentContextMode);
    var pass = localStorage.getItem('homer-sync-pass') || '';
    if(!hasJoeyRemoteAuth() || streaming || document.visibilityState === 'hidden') return Promise.resolve(null);
    if(joeySyncMetaInflight && joeySyncMetaInflightMode === activeMode) return joeySyncMetaInflight;
    var minInterval = getJoeySyncMetaMinInterval(opts.source);
    var lastCheckAt = joeyLastSyncMetaCheckAtByMode[activeMode] || 0;
    if(minInterval > 0 && (Date.now() - lastCheckAt) < minInterval) return Promise.resolve(null);
    joeyLastSyncMetaCheckAtByMode[activeMode] = Date.now();
    joeySyncMetaInflightMode = activeMode;
    joeySyncMetaInflight = joeyFetchJson(joeyActionUrl('sync-meta', pass, activeMode), withSupabaseAuth({ cache:'no-store' })).then(function(data){
      var syncMeta = data && data.syncMeta ? data.syncMeta : {};
      var remoteStamp = parseJoeyBundleStamp(syncMeta.updatedAt);
      var localStamp = getJoeyBundleStamp(activeMode);
      ...
      rememberJoeyBundleStamp(activeMode, syncMeta.updatedAt);
      return data;
    }).catch(function(){ return null; }).finally(function(){
      joeySyncMetaInflight = null;
      joeySyncMetaInflightMode = '';
    });
    return joeySyncMetaInflight;
  }
  ...
  function saveHistoryToRedis(opts){
    opts = opts || {};
    var pass = localStorage.getItem('homer-sync-pass') || '';
    if(!hasJoeyRemoteAuth() || !chatHistory.length) return Promise.resolve({ ok:false, skipped:true, reason:'unavailable' });
    var activeMode = getJoeyModeKey(currentContextMode);
    var signature = getJoeyHistorySaveSignature(activeMode, chatHistory);
    if(!opts.force && joeyLastHistorySaveSignatureByMode[activeMode] === signature){
      return Promise.resolve({ ok:true, skipped:true, reason:'unchanged' });
    }
    ...
    var request = fetch(joeyActionUrl('history', pass), withSupabaseAuth({
      method:'POST',
      headers:{'Content-Type':'application/json'},
      keepalive: !!opts.keepalive,
      body: JSON.stringify(withContextMode({
      ...
      if(joeyHistorySaveInflight[activeMode] && joeyHistorySaveInflight[activeMode].signature === signature){
        joeyHistorySaveInflight[activeMode] = null;
      }
    });
  ...
  function clearChatWindow(){
    ...
    var pass = localStorage.getItem('homer-sync-pass') || '';
    if(hasJoeyRemoteAuth()){
      fetch(joeyActionUrl('history', pass), withSupabaseAuth({
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body: JSON.stringify(withContextMode({ passphrase:pass, messages:[], chatClearedAt:getChatClearedAt(currentContextMode) }))
      })).catch(function(){});
    }
  }
  ...
  function fetchJoeyJson(url, opts){
    return fetch(url, withSupabaseAuth(opts || {})).then(function(r){
      return r.text().then(function(text){
        var data = {};
  ...
  function fetchContextFiles(){
    var pass = localStorage.getItem('homer-sync-pass') || '';
    if(!hasJoeyRemoteAuth()) return Promise.reject(new Error('Sign in or unlock Joey memory to access context files.'));
  ...
  function showContextFilesModal(files){
    ...
    modal.style.cssText = 'position:fixed;inset:0;background:rgba(8,0,26,.78);z-index:10030;display:flex;align-items:center;justify-content:center;padding:18px;';
    var panel = document.createElement('div');
    panel.style.cssText = 'width:min(1100px,96vw);height:min(86vh,900px);background:#07111f;border:1px solid rgba(160,150,200,.28);border-radius:18px;box-shadow:0 25px 70px rgba(0,0,0,.45);display:grid;grid-template-columns:260px 1fr;overflow:hidden;';
    var sidebar = document.createElement('div');
    sidebar.style.cssText = 'border-right:1px solid rgba(160,150,200,.18);padding:16px;overflow:auto;background:linear-gradient(180deg,rgba(13,0,32,.96),rgba(8,0,26,.96));';
    var content = document.createElement('div');
    content.style.cssText = 'padding:18px;overflow:auto;color:#e5eefb;';
    var title = document.createElement('div');
    title.style.cssText = 'display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;color:#f8f0ff;font-weight:800;';
    title.innerHTML = '<span>Canonical Context Files</span><button type="button" style="padding:6px 10px;border-radius:10px;border:none;background:#180030;color:#fff;cursor:pointer;">Close</button>';
    ...
    names.forEach(function(name, idx){
      ...
      btn.style.cssText = 'display:block;width:100%;text-align:left;margin:0 0 8px;padding:10px 12px;border-radius:12px;border:1px solid rgba(160,150,200,.16);background:rgba(13,0,32,.78);color:#dbeafe;cursor:pointer;font-weight:700;';
      btn.addEventListener('click', function(){
        content.innerHTML = '<div style="font-size:1rem;font-weight:800;margin-bottom:10px;color:#f8f0ff;">' + escHtml(name) + '</div><pre style="white-space:pre-wrap;word-break:break-word;background:rgba(13,0,32,.72);border-radius:14px;padding:16px;border:1px solid rgba(160,150,200,.18);line-height:1.5;font-size:.87rem;">' + escHtml(files[name] || '') + '</pre>';
        Array.from(sidebar.querySelectorAll('button')).forEach(function(el){ if(el !== title.querySelector('button')) el.style.background = 'rgba(13,0,32,.78)'; });
  ...
  function loadHistoryFromRedis(force){
    ...
    var pass = localStorage.getItem('homer-sync-pass') || '';
    if(!hasJoeyRemoteAuth()){ console.warn('[Joey] No sync credentials - cannot load history from shared store'); return; }
    fetch(joeyActionUrl('history', pass), withSupabaseAuth())
      .then(function(r){ return r.json(); })
      .then(function(data){
  ...
  function buildAutoLearnPayload(){
    var pass = localStorage.getItem('homer-sync-pass') || '';
    if(!hasJoeyRemoteAuth() || chatHistory.length < 2) return null;
  ...
  function runAutoLearnPayload(payload, reason){
    ...
    return fetch(joeyActionUrl('learn', '', payload.mode), withSupabaseAuth({
      method:'POST',
      headers:{'Content-Type':'application/json'},
      keepalive: true,
  ...
  document.addEventListener('visibilitychange', function(){
    if(document.visibilityState === 'hidden'){
      ...
      maybeRefreshJoeyBundleFromRedis({ source:'visibility-visible' });
    }
  });
  window.addEventListener('pagehide', function(){
  ...
  // ── Supabase Realtime: refresh Joey bundle when messages/memories change ──
  var joeyRealtimeRefreshTimer = null;
  function scheduleJoeyRealtimeRefresh(source){
    if(!hasJoeyRemoteAuth()) return;
  ...
  window.addEventListener('supabase:message', function(){ scheduleJoeyRealtimeRefresh('realtime-message'); });
  window.addEventListener('supabase:memory', function(){ scheduleJoeyRealtimeRefresh('realtime-memory'); });

  // On Supabase login: load Joey bundle immediately for Bogdan
  window.addEventListener('supabase:session', function(){
    if(!hasJoeyRemoteAuth()) return;
    setTimeout(function(){
      refreshJoeyBundleFromRedis({ forceMode:true, mode:currentContextMode, source:'supabase-session' }).catch(function(){
        loadHistoryFromRedis({ force:false, source:'supabase-session-fallback' });
      });
    }, 300);
  });

  // Load conversation from Redis/Supabase as fallback (cross-device continuity)
  setTimeout(function(){
  ...
  setInterval(function(){
    if(!hasJoeyRemoteAuth() || !panelOpen || streaming) return;
  ...
  function inferImplicitProfileRemember(text){
    var raw = String(text || '').trim();
    if(!raw) return null;
    var normalized = raw.replace(/\s+/g, ' ').trim();
    if(/^(?:please\s+)?(?:remember|save|store|keep\s+in\s+mind|note)\b/i.test(normalized)) return null;
    if(/^(?:my\s+favou?rite\s+(?:colou?r|coleor)\s+is\s+.+|i\s+prefer\s+.+|my\s+name\s+is\s+.+|i\s+(?:always|usually|never)\s+.+)$/i.test(normalized)){
      return {
        memory: clampAgentText(normalized, 600),
        category: /\bfavou?rite|prefer|always|usually|never\b/i.test(normalized) ? 'preference' : 'fact',
        source: 'implicit-profile-remember',
        pinned: true
      };
    }
    return null;
  ...
  function applyAgentRememberMutation(memData){
    var pass = localStorage.getItem('homer-sync-pass') || '';
    if(!hasJoeyRemoteAuth()) return Promise.resolve({ ok:false, message:'Sign in or unlock Joey memory so it can be updated.' });
    ...
    return fetch(rememberUrl, withSupabaseAuth({
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify(rememberBody)
    })).then(function(r){ return r.json(); }).then(function(d){
      if(d && d.ok){
        if(d.updatedAt) rememberJoeyBundleStamp(currentContextMode, d.updatedAt);
        refreshCanonicalFiles().catch(function(){});
        refreshJoeyBundleFromRedis({ source:'remember-action' }).catch(function(){});
  ...
  function executeActions(text, userText){
    var actions = [];
    var regex = /\[ACTION:(TASK|GOAL|HABIT|FOCUS|EVENT|REMEMBER|FORGET|PROJECT|NOTE|LINK|SECRET|COMMAND|JOURNAL)\]([\s\S]*?)(?:\[\/ACTION\]|$)/g;
    var match;
      ...
      try{
        if(a.type === 'TASK'){
          ...
          if(habitName){
            ...
            var HABIT_COLORS = ['#34d399','#FF0066','#f87171','#fbbf24','#a78bfa','#fb7185','#38bdf8','#dd44ff'];
          ...
          // Journal entry — create or append to a journal entry
          var jData;
          try{ jData = JSON.parse(a.data); }catch(e){ jData = {content: a.data}; }
          if(!jData.date) jData.date = new Date().toISOString().slice(0,10);
          try{ window.dispatchEvent(new CustomEvent('homer-journal-action', {detail: jData})); }catch(e){}
          notifications.push('Journal entry saved: ' + jData.date);
        }
    ...
    if(notifications.length){
      ...
      toast.style.cssText = 'background:rgba(204,0,255,.1);border:1px solid rgba(204,0,255,.25);font-size:.78rem;color:#dd44ff;text-align:center;max-width:100%;';
    ...
    return text.replace(/\[ACTION:(TASK|GOAL|HABIT|FOCUS|EVENT|REMEMBER|FORGET|PROJECT|NOTE|LINK|SECRET|COMMAND|JOURNAL)\][\s\S]*?(?:\[\/ACTION\]|$)/g, '').trim();
  }
  ...
  async function sendMessage(){
    ...
    (function(){
      try{
        ...
        // Compute mood trend for AI reflection themes
        var moodCounts = {};
        recent.forEach(function(e){ if(e.moodLabel){ moodCounts[e.moodLabel] = (moodCounts[e.moodLabel]||0)+1; } });
        var topMoods = Object.keys(moodCounts).sort(function(a,b){ return moodCounts[b]-moodCounts[a]; }).slice(0,3);
        var jSection = '\n\n=== MY RECENT JOURNAL (last 60 days, ' + recent.length + ' entries) ===\n- Use these to understand my current state of mind, mood, recent experiences, and recurring themes.\n- Reference them naturally when relevant — do not announce "I see in your journal...".\n- Notice patterns: recurring worries, wins, people mentioned, goals progressing or stalling.\n';
        if(topMoods.length) jSection += '- Dominant moods lately: ' + topMoods.join(', ') + '\n';
        recent.forEach(function(e){
          jSection += '\n[' + e.date + ']';
          if(e.moodLabel) jSection += ' ' + (e.mood||'') + ' ' + e.moodLabel;
          jSection += '\n' + (e.content||'').trim().slice(0, 800) + '\n';
        });
        systemPrompt += jSection;
      }catch(_){}
    })();
    systemPrompt += '\n\n=== PROJECT ACTIONS ===\n- If the user explicitly asks to create, rename, edit, archive, restore, or delete a project, append a PROJECT action.\n- PROJECT create format: [ACTION:PROJECT]{\"op\":\"create\",\"name\":\"Project Name\",\"description\":\"Optional description\",\"icon\":\"rocket\",\"color\":\"#FF0066\"}[/ACTION]\n- PROJECT update format: [ACTION:PROJECT]{\"op\":\"update\",\"project\":\"Apps\",\"name\":\"Apps Platform\",\"description\":\"Shared product work\",\"icon\":\"AP\",\"color\":\"#34d399\"}[/ACTION]\n- PROJECT archive format: [ACTION:PROJECT]{\"op\":\"archive\",\"project\":\"Apps\"}[/ACTION]\n- PROJECT restore format: [ACTION:PROJECT]{\"op\":\"restore\",\"project\":\"Apps\"}[/ACTION]\n- PROJECT delete format: [ACTION:PROJECT]{\"op\":\"delete\",\"project\":\"Apps\"}[/ACTION]\n- Use \"project\" to identify the existing project for any non-create action.\n- Never delete a project that still has issues.\n- Never place new tasks into archived projects.';
    ...
    var implicitProfileRemember = (!directRememberRequest && currentProvider === 'joey')
      ? inferImplicitProfileRemember(displayText)
      : null;
    ...
    var historyEntry = {role:'user', content:fullContent, displayText:displayText, provider:currentProvider, mode:currentContextMode, ts:Date.now()};
    ...
    if(currentProvider === 'joey'){
      Promise.resolve(refreshCanonicalFiles()).catch(function(){ return null; });
    }
    ...
    try{
      ...
      var res = await fetch(apiEndpoint, withSupabaseAuth({
        method: 'POST',
        headers: {'Content-Type':'application/json'},
      ...
      if(currentProvider === 'joey'){
        ...
        Promise.resolve(commitJoeyMemory({
          ...
          return Promise.resolve(triggerAutoLearn('post-reply')).catch(function(){ return null; });
        });
      } else {
        saveHistoryToRedis();
        triggerAutoLearn();
...
(function(){
  ...
  var CATS = {health:{icon:'💪',color:'#CC00FF'},finance:{icon:'💰',color:'#fbbf24'},career:{icon:'🏢',color:'#FF0066'},personal:{icon:'🌟',color:'#a78bfa'},education:{icon:'📚',color:'#f59e0b'},travel:{icon:'✈️',color:'#06b6d4'},creative:{icon:'🎨',color:'#ec4899'},relationship:{icon:'❤️',color:'#ef4444'}};
  ...
  function renderStats(goals){
    ...
    statsRow.innerHTML =
      '<div class="goals-stat-card"><div class="goals-stat-ring">'+ring(total?Math.round(completed/total*100):0,'#CC00FF')+'</div><div class="goals-stat-info"><h3>Achieved</h3><div class="stat-value">'+completed+'/'+total+'</div><div class="stat-sub">'+getGoalsCopy().completedSub+'</div></div></div>'+
      '<div class="goals-stat-card"><div class="goals-stat-ring">'+ring(avgProgress,'#FF0066')+'</div><div class="goals-stat-info"><h3>Avg Progress</h3><div class="stat-value">'+avgProgress+'%</div><div class="stat-sub">across all goals</div></div></div>'+
...
(function(){
  ...
  // Swipe-to-close removed — the handle is at the top of the sheet so any
  // downward scroll starting there triggered close. Close via backdrop tap.
