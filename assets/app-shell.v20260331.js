  ...
  resetCountdown();
    ...
    function applySavedQuoteMutation(request){
      ...
      if(op === 'clear'){
        saveSaved([]);
        renderSaved();
        return Promise.resolve({ ok:true, message:'Saved quotes cleared.' });
      }
      if(op === 'delete'){
        ...
        saveSaved(list);
        renderSaved();
        return Promise.resolve({ ok:true, message:'Quote removed.' });
      }
      if(op === 'update' || op === 'author'){
        ...
        if(op === 'author'){
          ...
          var parsedUpdate = parseSavedQuoteValue(req.quote || req.memory || '', req.author || list[targetIdx].a || 'Unknown');
          if(!parsedUpdate) return Promise.resolve({ ok:false, message:'Invalid quote update.' });
          list[targetIdx] = { q:parsedUpdate.q, a:parsedUpdate.a, ts:list[targetIdx].ts || Date.now() };
        }
        saveSaved(list);
        renderSaved();
        return syncSavedQuotesToJoeyFile('saved-quotes-edit', { skipBackups:false }).then(function(){
          return { ok:true, message:'Quote updated in Quotes.md.' };
        });
      }
      var parsed = parseSavedQuoteValue(req.quote || req.memory || '', req.author || 'Unknown');
      ...
      if(existingIdx >= 0){
        ...
        list[existingIdx].ts = list[existingIdx].ts || Date.now();
      } else {
        list.unshift({ q:parsed.q, a:parsed.a, ts:Date.now() });
      }
      saveSaved(list);
      renderSaved();
      return syncSavedQuotesToJoeyFile('saved-quotes-add', { skipBackups:false }).then(function(){
    ...
    function applySavedQuoteBatch(entries, reason){
      ...
      source.forEach(function(entry){
        var parsed = parseSavedQuoteValue(
      ...
      saveSaved(list);
      renderSaved();
      return syncSavedQuotesToJoeyFile(reason || 'saved-quotes-batch', { skipBackups:false }).then(function(){
        return {
          ok:true,
    ...
    function addCurrentToSaved(){
      ...
      if(!list.some(it=>it.q===q && it.a===a)){
        ...
        const prev=els.save.textContent; els.save.textContent="Saved Ã¢Å“â€œ"; setTimeout(()=>els.save.textContent=prev,1100);
      }
    }
...
(function(){
      ...
      if('serviceWorker'in navigator && location.protocol.startsWith('http')){
        const swCode=`const CACHE='homer-v10'; const APP_SHELL=['/','/manifest.json','/icon.svg','/icon-192.png','/icon-512.png'];
...
(function(){
  ...
  var LS_KEYS = [
    ...
    'homer-sync-pass', 'homer-sync-auto',
    'homer-auth-user', 'homer-auth-hash',
    'homer-cal-ics', 'homer-cal-events',
  ...
  function isLocalOnlyBackupKey(key){
    return TAB_COORD_KEYS.indexOf(key) >= 0 || LOCAL_ONLY_BACKUP_KEYS.indexOf(key) >= 0;
  }

  function buildBackupManifest(data, idbKeys){
    var lsKeys = Object.keys(data).filter(function(key){
      return IDB_KEYS.indexOf(key) < 0 && BACKUP_RUNTIME_KEYS.indexOf(key) < 0;
    }).sort();
    return {
  ...
  function getAll(){
    ...
    // Backup ALL localStorage keys to ensure nothing is missed
    for(var i=0;i<localStorage.length;i++){
  ...
  var DB_BACKUP_DELAY_PERSONAL = 4000;
  var DB_BACKUP_DELAY_WORK = 4000;
  ...
  var SYNC_INTERVAL = 1800000; // 30 min safety net; normal changes still sync via debounce
  ...
  var syncMetaKeys = ['homer-sync-pass','homer-sync-auto','homer-auth-hash', LEADER_KEY, LEADER_HB_KEY, FIELD_SYNC_CURSOR_KEY, FIELD_SYNC_DEVICE_KEY];
...
(function(){
  ...
  function refreshJoeySyncStatus(opts){
    ...
    joeySyncStatusInFlight = fetch('/api/gdrive-reconcile', {
      ...
      var summary = drift.inSync
        ? 'Redis and Google Drive are aligned for ' + modeLabel + ' mode.'
        : (compareResp.wouldChange
          ? 'Safe reconcile found missing context in Drive. Redis can absorb it without overwriting conflicts.'
          : 'Redis and Drive still differ in some buckets. Current Redis data is being preserved where values conflict.');
      var cards = [
        ...
        { label:'Drift', value: drift.inSync ? 'In sync' : (drift.mismatchCount + ' bucket(s) differ') },
        { label:'Safe Merge Preview', value: mergeAdds || 'No missing items detected' }
      ];
      var bucketLabels = {
        profile: 'Profile',
        memories: 'Memories',
      ...
      return { syncMeta:syncMeta, driveMeta:driveMeta, drift:drift, compare:compareResp };
    }).catch(function(err){
  ...
  document.getElementById('oc-gdrive-backup').addEventListener('click', function(){
    ...
    Promise.resolve(ensureSavedQuotesContext('gdrive-backup')).catch(function(){ return false; }).then(function(){
      return collectTaskSnapshot();
    }).then(function(tasks){
      var quoteMarkdown = (typeof buildSavedQuotesMarkdown === 'function' && typeof loadSaved === 'function')
  ...
  document.getElementById('oc-gdrive-restore').addEventListener('click', function(){
    ...
    fetch('/api/gdrive-restore', {
      ...
      if(d.ok){
        ...
        if(d.restored && d.restored.history) loadHistoryFromRedis(true);
        refreshCanonicalFiles().catch(function(){});
        if(libraryPanel && libraryPanel.classList.contains('open')) fetchFileLibrary({ silent:true });
  ...
  function loadChatFromVault(){
    if(typeof window._homerLoadVault === 'function' && window._homerVaultUnlocked){
      window._homerLoadVault().then(function(data){
        ...
        chatHistory = Array.isArray(next) ? next : [];
        renderMessages();
      });
    }
  }
  ...
  function mergeContextCustomFiles(files, customFiles){
    ...
    if(!merged[SAVED_QUOTES_FILE_NAME]){
      var savedQuotesMd = buildSavedQuotesMarkdown(loadSaved());
      if(savedQuotesMd) merged[SAVED_QUOTES_FILE_NAME] = savedQuotesMd;
    }
    return merged;
  }
  ...
  function loadHistoryFromRedis(force){
    ...
    fetch(joeyActionUrl('history', pass))
      ...
      .then(function(data){
        ...
        if(data.history && data.history.length && (force || !chatHistory.length)){
          chatHistory = data.history;
          renderMessages();
          saveChatToVault();
          console.log('[Joey] Loaded ' + data.history.length + ' messages from Redis' + (force ? ' (forced)' : ''));
        } else if((!data.history || !data.history.length) && force){
          chatHistory = [];
          renderMessages();
          saveChatToVault();
  ...
  document.addEventListener('visibilitychange', function(){
    if(document.visibilityState === 'hidden') triggerAutoLearn('visibility-hidden');
  });
  window.addEventListener('pagehide', function(){
  ...
  setTimeout(loadHistoryFromRedis, 1500);
  setInterval(function(){
    var pass = localStorage.getItem('homer-sync-pass') || '';
    ...
    return fetch('/api/gdrive-reconcile', {
      ...
      .then(function(d){
        if(d.ok){
          ...
          if(d.reconciled && d.reconciled.history) loadHistoryFromRedis(true);
          if(d.changed){
            refreshCanonicalFiles().catch(function(){});
            if(libraryPanel && libraryPanel.classList.contains('open')) fetchFileLibrary({ silent:true });
          }
  ...
  var JOEY_DRIVE_RECONCILE_INTERVAL_MS = 10 * 60 * 1000;
  ...
  function extractGeneratedQuotes(text){
    ...
    function pushQuote(value, author){
      var parsed = parseSavedQuoteValue(value, author || 'Unknown');
      if(!parsed) return;
      var key = normalizeSavedQuoteText(parsed.q).toLowerCase() + '|' + String(parsed.a || 'Unknown').trim().toLowerCase();
      if(seen[key]) return;
      seen[key] = true;
  ...
  async function sendMessage(){
    ...
    try{
      ...
      var res = await fetch(apiEndpoint, {
        method: 'POST',
        headers: {'Content-Type':'application/json'},
        body: JSON.stringify(withContextMode({ messages: apiMessages, passphrase: syncPass, systemPromptOverride: systemPrompt }))
      });
