...
(function () {
  ...
  var CSS = `
    ...
    .vh-wrap { }
    .vh-tabs { display:flex; gap:4px; padding-bottom:16px; border-bottom:1px solid rgba(55,53,47,.05); margin-bottom:20px; }
    ...
    .vh-tab.active { background:rgba(59,130,246,.15); border-color:rgba(59,130,246,.3); color:#9065b0; }
    .vh-tab:hover:not(.active) { background:rgba(55,53,47,.03); color:#9b8f82; }

    /* Today */
    .vh-today-header { display:flex; justify-content:space-between; align-items:center; margin-bottom:24px; gap:16px; flex-wrap:wrap; padding:20px 24px; background:rgba(55,53,47,.02); border:1px solid rgba(55,53,47,.05); border-radius:18px; }
    .vh-date { font-size:.72rem; color:#64748b; font-weight:700; text-transform:uppercase; letter-spacing:1px; margin-bottom:4px; }
    .vh-greeting-text { font-size:1.5rem; font-weight:900; color:#37352f; line-height:1.2; }
    .vh-motiv { font-size:.88rem; color:#9b8f82; margin-top:8px; }
    ...
    .vh-card { display:flex; align-items:center; gap:12px; padding:14px 16px 14px 20px; background:rgba(55,53,47,.02); border:1px solid rgba(55,53,47,.05); border-radius:14px; transition:background .2s, border-color .2s; position:relative; overflow:hidden; }
    .vh-card:hover { background:rgba(55,53,47,.04); }
    ...
    .vh-card-name { font-size:.92rem; font-weight:700; color:#37352f; margin-bottom:5px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    .vh-card-done .vh-card-name { color:#9b8f82; text-decoration:line-through; text-decoration-color:rgba(52,211,153,.5); }
    .vh-card-meta { display:flex; align-items:center; gap:6px; margin-bottom:7px; flex-wrap:wrap; }
    .vh-cat-pill { font-size:.68rem; padding:2px 8px; border-radius:20px; background:rgba(55,53,47,.05); color:#9b8f82; font-weight:600; }
    ...
    .vh-dot { width:9px; height:9px; border-radius:3px; background:rgba(55,53,47,.05); flex-shrink:0; }
    .vh-dot.done { background:#34d399; }
    .vh-dot.today { outline:2px solid #9065b0; outline-offset:1px; }
    .vh-dot.skip { background:rgba(55,53,47,.02); }
    .vh-card-action { flex-shrink:0; margin-left:4px; }
    .vh-check { width:38px; height:38px; border-radius:11px; border:2px solid rgba(55,53,47,.11); background:none; cursor:pointer; display:flex; align-items:center; justify-content:center; font-size:1.1rem; font-weight:900; color:#fff; transition:all .2s; font-family:inherit; }
    .vh-check:hover { border-color:rgba(55,53,47,.22); background:rgba(55,53,47,.05); transform:scale(1.06); }
    ...
    .vh-cnt-btn { width:30px; height:30px; border-radius:9px; border:1px solid rgba(55,53,47,.08); background:rgba(55,53,47,.04); color:#37352f; font-size:1.1rem; font-weight:700; cursor:pointer; display:flex; align-items:center; justify-content:center; transition:background .15s; line-height:1; font-family:inherit; }
    .vh-cnt-btn:hover { background:rgba(55,53,47,.08); }
    ...
    .vh-skip-chip { padding:5px 12px; border-radius:20px; font-size:.75rem; color:#64748b; background:rgba(55,53,47,.02); border:1px solid; }
    ...
    .vh-empty-today { text-align:center; padding:30px; color:#64748b; font-size:.88rem; background:rgba(55,53,47,.02); border-radius:12px; border:1px dashed rgba(55,53,47,.07); }
    .vh-all-done { text-align:center; padding:32px 20px; background:linear-gradient(135deg,rgba(52,211,153,.08),rgba(144,101,176,.08)); border:1px solid rgba(52,211,153,.2); border-radius:18px; margin-top:16px; }
    ...
    /* Buttons */
    .vh-btn-primary { padding:10px 24px; border-radius:12px; border:none; background:#7a5ab8; color:#fff; font-size:.88rem; font-weight:700; cursor:pointer; transition:background .2s; font-family:inherit; }
    .vh-btn-primary:hover { background:#6c4c9a; }
    .vh-btn-ghost { padding:10px 20px; border-radius:12px; border:1px solid rgba(55,53,47,.09); background:none; color:#9b8f82; font-size:.88rem; font-weight:600; cursor:pointer; transition:all .15s; font-family:inherit; }
    .vh-btn-ghost:hover { background:rgba(55,53,47,.04); color:#37352f; }

    /* Stats */
    .vh-stats-top { display:flex; align-items:center; gap:12px; margin-bottom:24px; flex-wrap:wrap; }
    .vh-select { padding:9px 12px; border-radius:10px; border:1px solid rgba(55,53,47,.09); background:#f9f8f6; color:#37352f; font-size:.88rem; outline:none; font-family:inherit; min-width:200px; cursor:pointer; }
    .vh-stat-cards { display:grid; grid-template-columns:repeat(4,1fr); gap:12px; margin-bottom:4px; }
    .vh-stat-card { padding:18px 12px; background:rgba(55,53,47,.02); border:1px solid rgba(55,53,47,.05); border-radius:16px; text-align:center; }
    ...
    .vh-hm-cell { width:13px; height:13px; border-radius:2px; background:rgba(55,53,47,.04); cursor:default; display:block; flex-shrink:0; transition:transform .1s; }
    .vh-hm-cell:hover { transform:scale(1.4); z-index:1; position:relative; }
    .vh-hm-cell.today { outline:1.5px solid #9065b0; outline-offset:1px; }
    .vh-hm-cell.skip { background:rgba(55,53,47,.01); }
    .vh-hm-cell.future { background:rgba(55,53,47,.01); cursor:default; }
    ...
    .vh-wp-pct { font-size:.6rem; color:#9b8f82; }

    /* Manage */
    .vh-manage-head { margin-bottom:20px; }
    .vh-manage-list { display:flex; flex-direction:column; gap:6px; }
    .vh-manage-row { display:flex; align-items:center; gap:10px; padding:12px 14px 12px 18px; background:rgba(55,53,47,.02); border:1px solid rgba(55,53,47,.05); border-radius:12px; position:relative; overflow:hidden; transition:opacity .2s; }
    ...
    .vh-mr-name { font-size:.88rem; font-weight:700; color:#37352f; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    .vh-mr-meta { font-size:.72rem; color:#64748b; margin-top:2px; }
    .vh-mr-actions { display:flex; gap:4px; flex-shrink:0; }
    .vh-mr-btn { width:32px; height:32px; border-radius:9px; border:1px solid rgba(55,53,47,.07); background:none; cursor:pointer; display:flex; align-items:center; justify-content:center; font-size:.9rem; transition:background .15s; font-family:inherit; }
    .vh-mr-btn:hover { background:rgba(55,53,47,.06); }
    .vh-mr-btn.del:hover { background:rgba(248,113,113,.15); border-color:rgba(248,113,113,.3); }
    .vh-archived-toggle { padding:13px 0; cursor:pointer; color:#64748b; font-size:.82rem; font-weight:700; display:flex; align-items:center; gap:8px; border-top:1px solid rgba(55,53,47,.04); margin-top:18px; user-select:none; }
    .vh-archived-toggle:hover { color:#9b8f82; }
    .vh-archived-list { display:flex; flex-direction:column; gap:6px; margin-top:10px; }

    /* Form */
    .vh-form { background:rgba(55,53,47,.02); border:1px solid rgba(55,53,47,.07); border-radius:18px; padding:22px; margin-bottom:22px; }
    .vh-form-title { font-size:1rem; font-weight:800; color:#37352f; margin-bottom:20px; }
    ...
    .vh-form-label { font-size:.72rem; font-weight:700; color:#9b8f82; text-transform:uppercase; letter-spacing:.5px; }
    .vh-form-hint { text-transform:none; font-weight:400; color:#64748b; letter-spacing:0; }
    .vh-input { padding:10px 14px; border-radius:10px; border:1px solid rgba(55,53,47,.08); background:#f9f8f6; color:#37352f; font-size:.88rem; outline:none; font-family:inherit; transition:border-color .2s; box-sizing:border-box; width:100%; }
    .vh-input:focus { border-color:#7a5ab8; }
    .vh-emoji-grid { display:flex; flex-wrap:wrap; gap:5px; }
    .vh-emoji-btn { width:34px; height:34px; border-radius:9px; border:1px solid rgba(55,53,47,.05); background:rgba(55,53,47,.02); cursor:pointer; font-size:1rem; display:flex; align-items:center; justify-content:center; transition:all .15s; font-family:inherit; }
    .vh-emoji-btn:hover { background:rgba(55,53,47,.07); transform:scale(1.12); }
    .vh-emoji-btn.active { border-color:#7a5ab8; background:rgba(59,130,246,.18); }
    ...
    .vh-color-btn.active { border-color:#fff; box-shadow:0 0 0 3px rgba(55,53,47,.13); }
    .vh-freq-row { display:flex; gap:20px; }
    .vh-freq-opt { display:flex; align-items:center; gap:7px; cursor:pointer; font-size:.85rem; color:#37352f; font-family:inherit; }
    ...
    .vh-day-btn span { display:inline-flex; align-items:center; justify-content:center; width:34px; height:34px; border-radius:9px; border:1px solid rgba(55,53,47,.08); background:rgba(55,53,47,.02); font-size:.78rem; font-weight:700; color:#9b8f82; cursor:pointer; transition:all .15s; }
    .vh-day-btn.active span { background:rgba(59,130,246,.2); border-color:#7a5ab8; color:#9065b0; }
    .vh-target-row { display:flex; align-items:center; gap:12px; }
    .vh-target-val { font-size:1.1rem; font-weight:800; color:#37352f; min-width:28px; text-align:center; }
  ...
  var COLORS = ['#34d399','#9065b0','#f87171','#fbbf24','#a78bfa','#fb7185','#38bdf8','#7acde8'];
  ...
  function buildToday(d) {
    ...
    if (!active.length) {
      return '<div class="vh-empty">' +
        ...
        '<h3 style="color:#37352f;margin:0 0 8px;font-weight:900">No habits yet</h3>' +
    ...
    var ringColor = allDone ? '#34d399' : '#7a5ab8';

    var ringHtml = '<svg width="90" height="90" viewBox="0 0 100 100">' +
      '<circle cx="50" cy="50" r="' + r + '" fill="none" stroke="rgba(55,53,47,.05)" stroke-width="8"/>' +
      ...
      '<text x="50" y="46" text-anchor="middle" fill="#37352f" font-size="20" font-weight="900" font-family="inherit">' + doneCount + '</text>' +
    ...
    if (notToday.length) {
      notTodayHtml = '<div class="vh-section-label" style="margin-top:22px">Not scheduled today</div>' +
        ...
        notToday.map(function(h) {
          return '<span class="vh-skip-chip" style="border-color:' + (h.color || '#f1efea') + '">' +
  ...
  function buildHabitCard(h, completions, today) {
    ...
    if (target > 1) {
      actionHtml = '<div class="vh-counter">' +
        ...
        '<span class="vh-cnt-val" style="color:' + (done ? color : '#9b8f82') + '">' + cnt + '/' + target + '</span>' +
  ...
  function buildStats(d) {
    ...
    var statCards = '<div class="vh-stat-cards">' +
      ...
      '<div class="vh-stat-card"><div class="vh-stat-val" style="color:#9065b0">' + total + '</div><div class="vh-stat-lbl">Total done</div></div>' +
    '</div>';
  ...
  function buildHeatmap(completions, h, color, target) {
      ...
      weeks.map(function(wk) {
        return '<div class="vh-hm-week">' +
          wk.map(function(cell) {
            var bg = '';
            if (cell.future) bg = 'rgba(55,53,47,.01)';
            else if (cell.done) bg = color;
            else if (cell.partial) bg = color.replace('#','') + '50';
            else if (cell.scheduled) bg = 'rgba(55,53,47,.04)';
            else bg = 'rgba(55,53,47,.01)';
            ...
            if (cell.isToday) style += 'outline:1.5px solid #9065b0;outline-offset:1px;';
    ...
    var legend = '<div class="vh-hm-legend">' +
      ...
      ['rgba(55,53,47,.04)', color + '55', color + 'aa', color].map(function(bg, i) {
