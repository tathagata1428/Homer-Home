...
function buildNemoClawSystemParts(context) {
  ...
  if (filesText) systemParts.push(clampText(filesText, forceFullContext ? 72000 : 32000));
...
export async function onRequest(context) {
  ...
  await handleJoeyGatewayRequest(req, res, {
    getProviderConfig({ mode, env: e }) {
      return {
        gatewayUrl: String(e.NEMOCLAW_GATEWAY_URL || 'http://localhost:11434').trim(),
        gatewayToken: String(e.NEMOCLAW_GATEWAY_TOKEN || '').trim(),
        primaryModel: mode === 'work'
          ? String(e.NEMOCLAW_WORK_MODEL || 'nemotron-3-super:cloud').trim()
          : String(e.NEMOCLAW_PERSONAL_MODEL || 'nemotron-3-super:cloud').trim(),
        fallbackModel: '',
        largeContext: true
      };
    },
    getFallbackGatewayConfig({ env: e }) {
      const gatewayUrl = String(e.OC_GATEWAY_URL || '').trim();
      const gatewayToken = String(e.OC_GATEWAY_TOKEN || '').trim();
      const primaryModel = String(e.OC_FALLBACK_MODEL || e.OC_MODEL || '').trim();
