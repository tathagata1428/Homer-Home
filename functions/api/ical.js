export async function onRequest(context) {
  ...
  if (!url.startsWith('https://')) {
    return Response.json({ error: 'Only HTTPS URLs are allowed' }, { status: 400, headers: corsHeaders });
  }
  ...
  try {
    ...
    if (!r.ok) {
      return Response.json({ error: 'Upstream returned ' + r.status }, { status: r.status, headers: corsHeaders });
    }
    ...
    return Response.json({ error: err.message || 'Failed to fetch calendar' }, { status: 500, headers: corsHeaders });
  }
}
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (line === 'BEGIN:VEVENT') {
      ...
      if (baseKey === 'SUMMARY') {
        ...
        if (key.indexOf('VALUE=DATE') >= 0 && val.length <= 8) evt.allDay = true;
        ...
        evt.attendees.push({ name: attName || '', email: attEmail || '', role: attRole, status: attStatus });
